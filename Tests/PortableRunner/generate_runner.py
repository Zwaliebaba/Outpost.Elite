...
because each suite defines helpers of its own (MakeCommander, Widen, and so on) in an
...
# state-cell table, and whatever the modernisation adds beside it. They used to be a hand-kept
# list here,
...
def support_sources(_repo: Path) -> list[Path]:
...
def write_unit(_out: Path, _source: Path, _suites: list[tuple[str, list[str]]]) -> tuple[str, int]:
    ...
    lines = [
        ...
        "#include <exception>",
        "",
        "// Defined in runner_main.cpp: the skip list OUTPOST_TEST_SKIP fills.",
...
def write_main(_out: Path, _entries: list[str], _total: int) -> None:
    ...
    `--coverage`, `--measure` and `--record` WERE HERE AND ARE NOT ANY MORE (M6-b-7). All three
    drove the interpreter: which labels a test ran, what a recording of every call would cost, and
    the fixture itself. There is no interpreter to drive.
    """
    lines = [
        ...
        "",
        "/*",
        " * A SKIP LIST, read from OUTPOST_TEST_SKIP -- one Suite.Method per line.",
        " *",
        " * The filter is a single substring and cannot say \"everything except these\", which is what",
        " * a measurement over a chosen subset needs.",
        " */",
    ...
    lines += [
        ...
        "  LoadSkipList();",
        "",
        "  // One optional substring of \"Suite.Method\".",
...
def write_makefile(_build: Path, _repo: Path, _here: Path, _generated: list[Path]) -> None:
    ...
    for source in support_sources(_repo):
        rule(source, "support")
