...
namespace GameLogicTests
{
  ...
  namespace
  {
    ...
    std::uint32_t CompareScreens(const Cpu6502& _cpu, std::uint16_t _base, const Elite::Canvas& _canvas, const std::wstring& _context)
    {
  ...
  public:
    ...
    TEST_METHOD(ShowingAMessageMatchesMESS)
    {
      ...
      Logger::WriteMessage(("MESS: " + std::to_string(compared) + " messages, " + std::to_string(erased) + " with one already up").c_str());
    }
  };
  ...
  public:
    ...
    TEST_METHOD(EveryDirectTokenMatchesTT27)
    {
       ...
       * and not a defect.
       */
      const auto reachesValueTokens = [](std::uint32_t _token) { return _token == 119u || _token == 125u || _token == 126u; };
