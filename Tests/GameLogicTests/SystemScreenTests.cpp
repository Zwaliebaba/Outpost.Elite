  ...
  TEST_METHOD(EverySystemsDataScreenMatchesTheShippedRoutine)
  {
    ...
    for (std::uint8_t galaxyNumber = 0; galaxyNumber < 8; ++galaxyNumber)
    {
      ...
      for (std::uint16_t system = 0; system < 256; ++system)
      {
        ...
        const std::wstring where = Widen("TT25: galaxy " + std::to_string(galaxyNumber + 1) + " system "
                                         + std::to_string(system) + " distance " + std::to_string(distance));
        ...
        cpu.memory[static_cast<std::uint16_t>(oracle.Label("QQ7") + 1)] =
          static_cast<std::uint8_t>(data.productivity >> 8);
        ...
        for (const Cpu6502::TrapHit& hit : cpu.trapHits)
        {
          if (hit.address == chpr)
          {
            expected.push_back(static_cast<std::uint32_t>(hit.a)
                               | (static_cast<std::uint32_t>(hit.watched[0]) << 8)
