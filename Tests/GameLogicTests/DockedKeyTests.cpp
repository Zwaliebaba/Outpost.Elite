...
const char* Name(KeyAction _action)
{
  switch (_action)
  {
    case KeyAction::Nothing: return "Nothing";
    case KeyAction::StatusMode: return "StatusMode";
    case KeyAction::LongRangeChart: return "LongRangeChart";
    case KeyAction::ShortRangeChart: return "ShortRangeChart";
    case KeyAction::DataOnSystem: return "DataOnSystem";
    case KeyAction::Inventory: return "Inventory";
    case KeyAction::MarketPrice: return "MarketPrice";
    case KeyAction::Launch: return "Launch";
    case KeyAction::EquipShip: return "EquipShip";
    case KeyAction::BuyCargo: return "BuyCargo";
    case KeyAction::DiskAccess: return "DiskAccess";
    case KeyAction::SellCargo: return "SellCargo";
    case KeyAction::ChangeView: return "ChangeView";
    case KeyAction::Hyperspace: return "Hyperspace";
    case KeyAction::ShowDistance: return "ShowDistance";
    case KeyAction::SearchBySystemName: return "SearchBySystemName";
    case KeyAction::HomeCrosshairs: return "HomeCrosshairs";
    case KeyAction::MoveCrosshairs: return "MoveCrosshairs";
    case KeyAction::CountdownOnly: return "CountdownOnly";
  }
  return "?";
}
} // namespace
...
public:
  TEST_METHOD(EveryKeyReachesTheSameLabelAsTheShippedRoutine)
  {
     ...
     */
    for (const std::uint8_t docked : { std::uint8_t{ 0xFF }, std::uint8_t{ 0x00 }, std::uint8_t{ 0x01 },
                                       std::uint8_t{ 0x40 } })
    {
          ...
          for (const bool held : { false, true })
          {
            for (std::uint16_t key = 0; key < 256; ++key)
            {
              ...
              const std::wstring where =
                Widen("TT102: key " + std::to_string(key) + (docked != 0 ? " docked" : " in flight")
                      + " view " + std::to_string(view) + " counter " + std::to_string(countdown)
                      + (held ? " H held" : ""));
              Assert::IsTrue(found, (where + L": the dispatch should reach a known label").c_str());

              const KeyAction expected = outcome[stoppedAt];
              const KeyOutcome got =
                Elite::ActionForKey(static_cast<std::uint8_t>(key), docked, view, countdown, held);

              Assert::AreEqual(static_cast<int>(expected), static_cast<int>(got.action),
                               (where + L": the game reaches " + Widen(Name(expected)) + L", the port says "
                                + Widen(Name(got.action)))
                                 .c_str());
