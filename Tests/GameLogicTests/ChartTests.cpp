...
namespace GameLogicTests
{
  ...
  public:
    ...
    TEST_METHOD(CrosshairStepMatchesTheShippedRoutine)
    {
      ...
      Logger::WriteMessage(
        ("TT123: " + std::to_string(compared) + " moves compared, " + std::to_string(refused) + " refused at the edge of the galaxy")
          .c_str());
    ...
    TEST_METHOD(TargetCrosshairsMatchTheShippedRoutine)
    {
      ...
      const std::array<std::uint32_t, 16> offsets = {0, 1, 36, 37, 38, 39, 100, 128, 200, 218, 219, 220, 221, 229, 230, 231};
      ...
      Logger::WriteMessage(
        ("TT103: " + std::to_string(compared) + " compared, of which " + std::to_string(drawn) + " drew something").c_str());
    ...
    TEST_METHOD(HyperspaceRequestMatchesTheShippedRoutine)
    {
                ...
                for (const std::uint32_t fuel : {0u, 10u, 70u, 255u})
                {
                  for (const std::uint32_t cursor : {20u, 40u, 96u, 200u})
                  {
                    ...
                    const std::wstring where = Where(L"hyp", chart) + L" docked=" + std::to_wstring(docked) + L" count=" +
                                               ...
                                               std::to_wstring(galaxyNumber);

                    Assert::AreEqual(wentGalactic, outcome == Elite::JumpOutcome::Galactic, (where + L": the galactic branch").c_str());
                    ...
                    Assert::IsTrue(
                      port.screen.characters == expected,
                     ...
                     */
                    Assert::AreEqual<std::uint32_t>(cpu.memory[oracle.Label("XC")], port.text.column, (where + L": XC").c_str());
                    Assert::AreEqual<std::uint32_t>(cpu.memory[oracle.Label("YC")], port.text.row, (where + L": YC").c_str());
                    ...
                    if (outcome == Elite::JumpOutcome::CountingDown || outcome == Elite::JumpOutcome::OutOfRange)
                    {
                      ...
                      {
                        Assert::AreEqual<std::uint32_t>(cpu.memory[static_cast<std::uint16_t>(zp.safehouse + index)],
                                                        jump.target.bytes[index],
      ...
      const char* names[] = {" docked=", " busy=", " galactic=", " already-there=", " out-of-range=", " counting-down="};
        ...
        for (int system = 0; system < 256; ++system)
        {
          ...
          for (const std::uint32_t caseFlags : {0x80u, 0xC0u})
          {
            ...
            const std::wstring where =
              L"galaxy " + std::to_wstring(galaxyNumber) + L" system " + std::to_wstring(system) + L" QQ17=" + std::to_wstring(caseFlags);
