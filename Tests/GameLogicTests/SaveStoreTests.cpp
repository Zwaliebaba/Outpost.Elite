...
namespace GameLogicTests
{
  ...
  public:
    ...
    TEST_METHOD(AWrittenCommanderComesBackByteForByte)
    {
      ...
      for (std::size_t index = 0; index < file.size(); ++index)
      {
        Assert::AreEqual(file[index], static_cast<std::uint8_t>(raw[index]), (L"file byte " + std::to_wstring(index)).c_str());
      }
    }
    ...
    TEST_METHOD(AnUnsafeNameIsRefusedRatherThanRewritten)
    {
      ...
      {
        Assert::IsTrue(store.Write(Name(allowed), file), (Widen(std::string("allowed: ") + allowed)).c_str());
      }
    }
    ...
    {
      for (const wchar_t* device : {L"CON",  L"PRN",  L"AUX",  L"NUL",  L"COM1", L"COM2", L"COM3", L"COM4", L"COM5", L"COM6", L"COM7",
      ...
      for (const wchar_t* file : {L"", L"C", L"CO", L"CONN", L"COM", L"COM0", L"LPT", L"LPT0", L"JAMESON", L"A", L"NULL"})
      {
