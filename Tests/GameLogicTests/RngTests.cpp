...
{
  return std::wstring(_what) + L" disagreed at iteration " + std::to_wstring(_iteration) + L": oracle "
         + std::to_wstring(_expected) + L", port " + std::to_wstring(_actual);
}
} // namespace
...
public:
  ...
  static void CompareSequence(const std::array<std::uint8_t, 4>& _seed, bool _carryIn, std::uint32_t _iterations)
  {
    ...
    for (std::uint32_t iteration = 0; iteration < _iterations; ++iteration)
    {
      ...
      Assert::AreEqual<std::uint32_t>(cpu.c ? 1u : 0u, actual.carry ? 1u : 0u,
                                      Describe(L"C", iteration, cpu.c, actual.carry).c_str());
  ...
  TEST_METHOD(EveryStartingStateAgreesForTheFirstCall)
  {
    ...
    {
      const std::array<std::uint8_t, 4> state = {
        static_cast<std::uint8_t>(seed & 0xFFu),
        static_cast<std::uint8_t>((seed >> 4) & 0xFFu),
      ...
      for (const bool carryIn : { false, true })
      {
        ...
        Assert::AreEqual<std::uint32_t>(cpu.c ? 1u : 0u, actual.carry ? 1u : 0u,
                                        Describe(L"C", seed, cpu.c, actual.carry).c_str());
      }
    }
  }
