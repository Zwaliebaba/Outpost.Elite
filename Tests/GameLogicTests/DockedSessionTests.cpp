...
public:
  ...
  void BeepAndPause() override { Note("beep"); }
  ...
  void WaitFrames(std::uint8_t _frames) override { Note("wait " + std::to_string(_frames)); }
  void FlushKeyboard() override { Note("flush"); }

  // 6502: RESET, RES2, ZEKTRAN, startat, stopat, LAUN and TITLE.
  void ResetUniverse() override { Note("reset"); }
  void ResetShip() override { Note("res2"); }
  void ClearKeyLogger() override { Note("zektran"); }
  void StartTheme() override { Note("music on"); }
  void StopTheme() override { Note("music off"); }
  void ShowDockingTunnel() override { Note("tunnel"); }
  ...
  void Run(std::uint8_t _code) override { Note("code " + std::to_string(_code)); }
...
private:
  void Note(std::string _what) { log.push_back(std::move(_what)); }
};
  ...
  switch (outcome.action)
  {
    ...
    case Elite::KeyAction::DiskAccess:
    {
      const Elite::DiskMenuResult menu =
        Elite::DiskAccessMenu(_game.save, _game.commander, _game.name, _game.image, _game.buffer,
                              _game.useDisk);
     ...
     * had wired them up.
     */
    case Elite::KeyAction::Launch: return "[launch: phase 3]";
    case Elite::KeyAction::ChangeView: return "[view " + std::to_string(outcome.view) + ": phase 3]";
    case Elite::KeyAction::Hyperspace: return "[hyperspace: phase 3]";
    case Elite::KeyAction::LongRangeChart: return "[long-range chart: needs a canvas]";
    case Elite::KeyAction::ShortRangeChart: return "[short-range chart: needs a canvas]";
    case Elite::KeyAction::ShowDistance: return "[distance: needs a chart]";
    case Elite::KeyAction::SearchBySystemName: return "[search: needs a chart]";
    case Elite::KeyAction::HomeCrosshairs: return "[home: needs a chart]";
    case Elite::KeyAction::MoveCrosshairs: return "[move: needs a chart]";
    case Elite::KeyAction::CountdownOnly: return "[countdown: phase 3]";
    case Elite::KeyAction::Nothing: return "nothing";
  }
...
public:
  ...
  TEST_METHOD(EveryDockedScreenIsReachableInOneSession)
  {
    ...
    Assert::AreEqual(static_cast<int>(Elite::KeyAction::StatusMode),
                     static_cast<int>(begun.outcome.action), L"a new game opens on the status screen");
    Assert::AreEqual(static_cast<int>(Elite::MainLoop::Docked), static_cast<int>(begun.loop),
                     L"and in the docked half of the main loop");
     ...
     */
    const std::uint32_t quotedFoodPrice =
      Elite::TotalPrice(Elite::MarketPrice(0, game->current.economy, game->market.randomiser), 1);
    ...
    game->keys = ScriptedKeys({
      '2', 13,                  // buy: two tonnes of the first item with any stock
      13, 13, 13, 13, 13,       // then nothing of the next five, which the routine accepts silently
      'Q',                      // and then a letter, which is gnum's only way out of the buy loop
      'N', 'N', // sell: refuse both items in the hold
    ...
    for (const Step& step : SCRIPT)
    {
      ...
      {
        Assert::IsTrue(game->sink.characters > 0,
                       (where + L": " + Widen(reached) + L" printed nothing at all").c_str());
        ++screensThatPrinted;
      }
    }
    ...
    Assert::AreEqual<std::uint8_t>(
      7, game->commander.bytes[static_cast<std::size_t>(Elite::Field::CargoHold)],
      L"five tonnes of food in the hold, plus the two just bought");
    Assert::IsTrue(game->commander.Cash() < startingCash,
                   L"and the money for them has left the commander");
  ...
  TEST_METHOD(TheScreensDoNotPrintTheSameThingAsEachOther)
  {
    ...
    for (const std::uint8_t key : { Elite::KEY_STATUS, Elite::KEY_MARKET_PRICE, Elite::KEY_BUY_CARGO,
    ...
    {
      Assert::IsTrue(drawn[left].text.size() > 20,
                     (Widen(drawn[left].what) + L" printed suspiciously little").c_str());
      for (std::size_t right = left + 1; right < drawn.size(); ++right)
      {
        Assert::AreNotEqual(drawn[left].text, drawn[right].text,
                            (Widen(drawn[left].what) + L" and " + Widen(drawn[right].what)
                             + L" printed the same thing")
                              .c_str());
      }
    }
