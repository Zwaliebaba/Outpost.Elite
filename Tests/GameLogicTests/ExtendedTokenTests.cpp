...
{
  return L"DTW2=" + std::to_wstring(_state.sentenceStart) + L" DTW5=" + std::to_wstring(_state.bufferLength)
         + L" DTW8=" + std::to_wstring(_state.caseMask) + L" out=" + Describe(_state.screen) + L" buf="
         + Describe(_state.buffer);
}
...
void CompareThroughDasc(const std::vector<std::uint8_t>& _text, const TextStateBytes& _state, const wchar_t* _label)
{
  ...
  for (std::size_t index = 0; index < _text.size(); ++index)
  {
    ...
    const std::wstring where = std::wstring(_label) + L", character " + std::to_wstring(index) + L" ("
                               + std::to_wstring(character) + L")";
...
public:
  ...
  TEST_METHOD(EveryControlCodeMatchesTheShippedDispatch)
  {
    ...
    for (const TextStateBytes& start : { plain, lowered })
    {
      for (std::uint8_t code = 1; code <= 31; ++code)
      {
         ...
         */
        const std::wstring where = L" for control code " + std::to_wstring(code) + L" from DTW1="
                                   + std::to_wstring(start.lowerCaseBits);
        ...
        Assert::AreEqual<std::uint32_t>(cpu.memory[oracle.Label("QQ17")], port.recursive.CaseFlags(),
                                        (L"QQ17 differs" + where).c_str());
        ...
        if (port.screen.characters != expected)
        {
          Assert::Fail((L"output differs" + where + L"\n  game: " + Describe(expected) + L"\n  port: "
                        + Describe(port.screen.characters))
                         .c_str());
        }
  ...
  TEST_METHOD(TheMessageBufferNeverFlushesOnItsOwn)
  {
    ...
    TextStateBytes state;
    state.justify = 0xC0;
    CompareThroughDasc(Text("DOCKED\x0cSTILL DOCKED, AND STILL NOT FLUSHED EVEN PAST THIRTY COLUMNS\x0c"), state,
                       L"message buffer");
  }
    ...
    for (std::uint32_t token = 1; token <= OVERRIDE_ENTRY_COUNT; ++token)
    {
      ...
      if (port.screen.characters != expected)
      {
        Assert::Fail((L"system override " + std::to_wstring(token) + L" differs\n  game: " + Describe(expected)
                      + L"\n  port: " + Describe(port.screen.characters))
                       .c_str());
      }
      ++compared;
