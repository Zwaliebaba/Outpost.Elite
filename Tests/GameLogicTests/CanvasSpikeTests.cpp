...
struct Change
{
  ...
  [[nodiscard]] std::uint8_t Mask() const noexcept { return static_cast<std::uint8_t>(before ^ after); }
};
...
public:
  ...
  TEST_METHOD(PixelMasksStraddleMulticolourPixelBoundaries)
  {
    ...
    Assert::IsTrue(straddling > 0,
                   L"PIXEL's masks are expected to straddle multicolour pixel boundaries. If this ever fails, the C64 "
