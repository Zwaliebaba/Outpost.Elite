...
namespace GameLogicTests
{
  ...
  public:
    ...
    TEST_METHOD(TheKillTallyMatchesEXNO2)
    {
      ...
      const Start STARTS[] = {
        {0u, 0u, 0u}, {200u, 0u, 0u}, {255u, 0u, 0u}, {0u, 255u, 0u}, {250u, 255u, 3u}, {128u, 200u, 0u},
      };
        ...
        for (const std::uint8_t shield : SHIELDS)
        {
          for (const std::uint8_t banks : BANKS)
          {
            for (std::uint8_t shape = 0; shape < 4u; ++shape)
            {
              ...
              cpu.memory[static_cast<std::uint16_t>(inf + 1u)] = static_cast<std::uint8_t>(block >> 8);
              ...
              const std::wstring where =
                WidenText("OOPS(damage " + std::to_string(damage) + ", shield " + std::to_string(shield) + ", banks " +
                          std::to_string(banks) + (fromBehind ? ", aft" : ", fore") + (carryIn ? ", carry set)" : ", carry clear)"));
