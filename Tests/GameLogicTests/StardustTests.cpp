...
struct DustLabels
{
  ...
  explicit DustLabels(const OracleImage& _oracle)
  {
    ...
    xx = _oracle.Label("XX");     yy = _oracle.Label("YY");
    newzp = _oracle.Label("newzp"); rand = _oracle.Label("RAND");
    alpha = _oracle.Label("ALPHA"); alp1 = _oracle.Label("ALP1"); alp2 = _oracle.Label("ALP2");
    beta = _oracle.Label("BETA");   bet1 = _oracle.Label("BET1"); bet2 = _oracle.Label("BET2");
    delta = _oracle.Label("DELTA"); delt4 = _oracle.Label("DELT4");
    rat = _oracle.Label("RAT");     rat2 = _oracle.Label("RAT2");
    p = _oracle.Label("P"); q = _oracle.Label("Q"); r = _oracle.Label("R");
    s = _oracle.Label("S"); t = _oracle.Label("T");
    x1 = _oracle.Label("X1"); y1 = _oracle.Label("Y1"); zz = _oracle.Label("ZZ");
...
void SeedField(Cpu6502& _cpu, Elite::Stardust& _dust, const DustLabels& _at, std::uint8_t _count,
               std::uint32_t _seed)
{
  _dust.count = _count;
  ...
  for (std::size_t slot = 0; slot < Elite::STARDUST_SLOTS; ++slot)
  {
    ...
    const std::uint8_t values[6] = { next(), next(), next(), next(),
                                     static_cast<std::uint8_t>(next() | 0x20u), next() };
    _dust.x[slot] = values[0];    _cpu.memory[static_cast<std::uint16_t>(_at.sx + slot)] = values[0];
    _dust.xLow[slot] = values[1]; _cpu.memory[static_cast<std::uint16_t>(_at.sxl + slot)] = values[1];
    _dust.y[slot] = values[2];    _cpu.memory[static_cast<std::uint16_t>(_at.sy + slot)] = values[2];
    _dust.yLow[slot] = values[3]; _cpu.memory[static_cast<std::uint16_t>(_at.syl + slot)] = values[3];
    _dust.z[slot] = values[4];    _cpu.memory[static_cast<std::uint16_t>(_at.sz + slot)] = values[4];
    _dust.zLow[slot] = values[5]; _cpu.memory[static_cast<std::uint16_t>(_at.szl + slot)] = values[5];
  }
}

void CompareField(const Cpu6502& _cpu, const Elite::Stardust& _dust, const DustLabels& _at,
                  const std::wstring& _where)
{
  for (std::size_t slot = 0; slot < Elite::STARDUST_SLOTS; ++slot)
  {
    const std::wstring which = _where + L" slot " + std::to_wstring(slot);
    Assert::AreEqual(_cpu.memory[static_cast<std::uint16_t>(_at.sx + slot)], _dust.x[slot],
                     (which + L": SX").c_str());
    Assert::AreEqual(_cpu.memory[static_cast<std::uint16_t>(_at.sxl + slot)], _dust.xLow[slot],
                     (which + L": SXL").c_str());
    Assert::AreEqual(_cpu.memory[static_cast<std::uint16_t>(_at.sy + slot)], _dust.y[slot],
                     (which + L": SY").c_str());
    Assert::AreEqual(_cpu.memory[static_cast<std::uint16_t>(_at.syl + slot)], _dust.yLow[slot],
                     (which + L": SYL").c_str());
    Assert::AreEqual(_cpu.memory[static_cast<std::uint16_t>(_at.sz + slot)], _dust.z[slot],
                     (which + L": SZ").c_str());
    Assert::AreEqual(_cpu.memory[static_cast<std::uint16_t>(_at.szl + slot)], _dust.zLow[slot],
                     (which + L": SZL").c_str());
  }
}
...
void SeedFlight(Cpu6502& _cpu, Elite::FlightState& _state, const DustLabels& _at,
...
void CompareFlight(const Cpu6502& _cpu, const Elite::FlightState& _state, const DustLabels& _at,
                   const std::wstring& _where)
{
  ...
  static const wchar_t* NAMES[] = { L"ALPHA", L"ALP2", L"ALP2+1", L"BET2", L"BET2+1",
                                    L"RAT",   L"RAT2" };

  for (std::size_t byte = 0; byte < std::size(BYTES); ++byte)
  {
    Assert::AreEqual(_cpu.memory[BYTES[byte].first], BYTES[byte].second,
                     (_where + L": " + NAMES[byte]).c_str());
  }
}

void SeedRandom(Cpu6502& _cpu, Elite::Rng& _rng, const DustLabels& _at,
                const std::array<std::uint8_t, 4>& _seed)
{
...
void CompareRandom(const Cpu6502& _cpu, const Elite::Rng& _rng, const DustLabels& _at,
                   const std::wstring& _where)
{
  for (std::size_t byte = 0; byte < 4u; ++byte)
  {
    ...
    for (const std::uint8_t a : EDGES)
    {
      for (const std::uint8_t operand : EDGES)
      {
        for (int which = 0; which < 8; ++which)
        {
          ...
          cpu.memory[at.delta] = operand;      flight.delta = operand;
          cpu.memory[at.alp1] = operand;       flight.alp1 = operand;
          cpu.memory[at.q] = operand;          math.q = operand;
          ...
          const std::wstring where = std::wstring(name) + L"(a=" + std::to_wstring(a) + L", x="
                                     + std::to_wstring(operand) + L")";
...
private:
  ...
  void RunAndCompare(const char* _label, std::uint8_t _view, bool _through)
  {
     ...
     */
    for (const std::uint32_t field : { 0x2C6A91B7u, 0x8D14E703u, 0x51F3A2C9u, 0xA07B5E26u,
                                       0x3FCD0841u })
    {
    for (const std::uint8_t count : { 3, 12 })
    {
      for (const Flight& flight : Flights())
      {
        ...
        for (int frame = 0; frame < frames; ++frame)
        {
          ...
          const std::wstring where = name
                                     + Widen(" view=" + std::to_string(_view) + " count="
                                             + std::to_string(count) + " frame="
                                             + std::to_string(frame) + " ")
                                     + flight.what;
    ...
    Assert::AreEqual<std::uint32_t>(static_cast<std::uint32_t>(5 * 2 * 7 * frames), compared,
                                    L"every field, every flight, every frame");
    ...
    Logger::WriteMessage(
      (entry + ": " + std::to_string(compared) + " frames, " + std::to_string(marked)
       + " marked bytes")
        .c_str());
  }
};
