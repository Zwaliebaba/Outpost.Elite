...
namespace GameLogicTests
{
  ...
  public:
    ...
    TEST_METHOD(MarketTableMatchesTheImage)
    {
      ...
      {
        Assert::AreEqual<std::uint32_t>(cpu.memory[static_cast<std::uint16_t>(table + offset)], Elite::MARKET_TABLE[offset],
    ...
    TEST_METHOD(EconomyAdjustmentMatchesTheShippedRoutine)
    {
      ...
      for (std::uint32_t value = 0; value < 256; ++value)
      {
        for (std::uint32_t economy = 0; economy < 8; ++economy)
        {
          ...
          const std::uint8_t ours = Elite::EconomyAdjustment(static_cast<std::uint8_t>(value), static_cast<std::uint8_t>(economy));
        ...
        for (std::uint32_t economy = 0; economy < 8; ++economy)
        {
          for (std::uint32_t randomiser = 0; randomiser < 256; ++randomiser)
          {
            ...
            const std::uint8_t ours =
              Elite::MarketPrice(static_cast<int>(item), static_cast<std::uint8_t>(economy), static_cast<std::uint8_t>(randomiser));
    ...
    TEST_METHOD(GeneratedMarketsMatchTheShippedRoutine)
    {
      ...
      for (std::uint32_t economy = 0; economy < 8; ++economy)
      {
        ...
        {
          const std::array<std::uint8_t, 4> state = {static_cast<std::uint8_t>(seed * 7u + 1u), static_cast<std::uint8_t>(seed * 13u + 5u),
          ...
          {
            Assert::AreEqual<std::uint32_t>(cpu.memory[static_cast<std::uint16_t>(avl + item)], market.availability[item],
                                            (where + L": availability of item " + std::to_wstring(item)).c_str());
          }

          Assert::AreEqual<std::uint32_t>(0u, market.availability[16], (where + L": Alien Items must be zeroed, as var does").c_str());
          ++compared;
        }
      }
    ...
    TEST_METHOD(SpendingAndReceivingCashMatchTheShippedRoutines)
    {
      ...
      for (const std::uint32_t cash : {0u, 1u, 255u, 256u, 1000u, 65535u, 65536u, 100000u, 0xFFFFFFFFu})
      {
        for (const std::uint32_t amount : {0u, 1u, 255u, 256u, 999u, 1000u, 1001u, 65535u})
        {
          for (const bool spending : {true, false})
          {
            ...
            const std::uint32_t after = (static_cast<std::uint32_t>(cpu.memory[cashAt]) << 24) |
      ...
      Logger::WriteMessage(
        ("LCASH and MCASH: " + std::to_string(compared) + " compared, " + std::to_string(refused) + " purchases refused").c_str());
      ...
      for (std::uint32_t price = 0; price < 256; ++price)
      {
        for (const std::uint32_t quantity : {0u, 1u, 2u, 3u, 17u, 64u, 100u, 127u, 128u, 200u, 255u})
        {
          ...
          Assert::AreEqual<std::uint32_t>(expected,
                                          Elite::TotalPrice(static_cast<std::uint8_t>(price), static_cast<std::uint8_t>(quantity)),
                                          (L"GCASH(" + std::to_wstring(price) + L", " + std::to_wstring(quantity) + L")").c_str());
          ++compared;
        }
      }
    ...
    TEST_METHOD(CargoCapacityMatchesTheShippedRoutine)
    {
            ...
            for (const std::uint32_t tribbles : {0u, 1024u})
            {
              for (const std::uint32_t item : {0u, 1u, 6u, 12u, 13u, 14u, 16u})
              {
                for (std::uint32_t amount = 0; amount < 256; ++amount)
                {
                  ...
                  const bool ourFits = Elite::CargoFits(block, static_cast<std::uint8_t>(item), static_cast<std::uint8_t>(amount));

                  Assert::AreEqual(gameFits, ourFits,
                                   (L"tnpr(item=" + std::to_wstring(item) + L" amount=" + std::to_wstring(amount) + L" filled=" +
                                    std::to_wstring(filled) + L" capacity=" + std::to_wstring(capacity) + L" tribbles=" +
                                    std::to_wstring(tribbles) + L")")
                                     .c_str());
      ...
      Logger::WriteMessage(
        ("tnpr: " + std::to_string(compared) + " capacity checks compared, " + std::to_string(fitted) + " fitted").c_str());
    ...
    TEST_METHOD(TheMarketScreenMatchesTheShippedRoutine)
    {
      ...
      for (std::uint32_t economy = 0; economy < 8; ++economy)
      {
        for (const std::uint32_t randomiser : {0u, 1u, 37u, 128u, 200u, 255u})
        {
          ...
          const std::wstring where = L"TT167 (economy " + std::to_wstring(economy) + L", randomiser " + std::to_wstring(randomiser) + L")";
