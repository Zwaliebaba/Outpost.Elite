  ...
  TEST_METHOD(EveryOpcodeTheInterpreterRunsIsPricedAndNothingElseIs)
  {
    ...
    for (int value = 0; value < 256; ++value)
    {
      ...
      Assert::AreEqual(isPriced, runs,
                       Widen("opcode " + Hex(opcode) + (runs ? " executes but is unpriced"
                                                             : " is priced but does not execute")).c_str());
      priced += isPriced ? 1 : 0;
      executed += runs ? 1 : 0;
    }
...
public:
  ...
  TEST_METHOD(EveryIndexedReadPaysExactlyOneForCrossing)
  {
    ...
    for (const std::uint8_t opcode : ABSOLUTE_INDEXED)
    {
      ...
      Assert::AreEqual<std::uint64_t>(1, CrossingPenaltyOf(opcode, false),
                                      Widen(Hex(opcode) + " crossing a page").c_str());
    }

    for (const std::uint8_t opcode : INDIRECT_INDEXED)
    {
      Assert::IsTrue(Cpu6502::PaysPageCrossPenalty(opcode), Widen(Hex(opcode) + " should pay").c_str());
      Assert::AreEqual<std::uint64_t>(1, CrossingPenaltyOf(opcode, true),
                                      Widen(Hex(opcode) + " crossing a page").c_str());
    }
...
public:
  ...
  TEST_METHOD(ABranchCostsTwoThreeOrFour)
  {
    ...
    static constexpr Case CASES[] = {
      { 0x10, "BPL", false, false, false, false },
      { 0x30, "BMI", true, false, false, false },
      { 0x50, "BVC", false, false, false, false },
      { 0x70, "BVS", false, true, false, false },
      { 0x90, "BCC", false, false, false, false },
      { 0xB0, "BCS", false, false, true, false },
      { 0xD0, "BNE", false, false, false, false },
      { 0xF0, "BEQ", false, false, false, true },
    };
  ...
  {
    static constexpr std::uint8_t CODE[] = {
      0x20, 0x00, 0x30,  // JSR $3000  -- trapped
      0x60,              // RTS
    };
...
public:
  ...
  TEST_METHOD(TheSeedTwistAndTheRandomiserCostWhatTheyCost)
  {
     ...
     */
    {
      ...
      Assert::AreEqual<std::uint16_t>(1, static_cast<std::uint16_t>(oracle.Label("DORND")
                                                                   - oracle.Label("DORND2")),
