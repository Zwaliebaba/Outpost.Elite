...
namespace GameLogicTests
{
  ...
  namespace
  {
    ...
    struct Labels
    {
      ...
      explicit Labels(const OracleImage& _oracle)
      {
        ...
        screen =
          static_cast<std::uint16_t>((image.memory[_oracle.Label("ylookupl")] | (image.memory[_oracle.Label("ylookuph")] << 8)) - 0x20);
      }
    };
    ...
    std::uint32_t CompareScreens(const Cpu6502& _cpu, std::uint16_t _base, const Elite::Canvas& _canvas, std::uint8_t _marker,
  ...
  public:
    ...
    TEST_METHOD(OneBarMatchesDIL)
    {
      ...
      {
        const std::uint16_t address = static_cast<std::uint16_t>(static_cast<int>(oracle.Label(entry.label)) + entry.offset);
        ...
        for (const std::uint8_t threshold : THRESHOLDS)
        {
          for (const auto& pair : COLOURS)
          {
            ...
            {
              const std::uint16_t start = static_cast<std::uint16_t>(Elite::DASHBOARD_BITMAP + 8u * 6u);
    ...
    TEST_METHOD(OneIndicatorMatchesDIL2)
    {
      ...
      for (std::uint32_t value = 0; value < 256; ++value)
      {
        const std::uint16_t start = static_cast<std::uint16_t>(Elite::DASHBOARD_BITMAP + 8u * 30u);
    ...
    TEST_METHOD(TheBulbsMatchECBLBandSPBLB)
    {
      ...
      for (const Bulb& bulb : BULBS)
      {
        for (int times = 1; times <= 3; ++times)
        {
          ...
          Assert::AreEqual<std::uint32_t>((times % 2 == 0) ? 0u : 2u, drawn, (where + L": how many cells are lit").c_str());
          marked += drawn;
        }
      }
  ...
  public:
    ...
    TEST_METHOD(TheWholeDashboardMatchesDIALS)
    {
      ...
      Assert::AreEqual<std::uint32_t>(static_cast<std::uint32_t>(CASES.size()) * 6u * 2u, compared, L"the whole sweep ran");
      ...
      Logger::WriteMessage(
        ("DIALS: " + std::to_string(compared) + " dashboards, " + std::to_string(energyPasses) + " with the energy bars").c_str());
    }
  };
