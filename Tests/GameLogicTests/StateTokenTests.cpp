  ...
  public:
    ...
    TEST_METHOD(EveryStateControlCodeMatches)
    {
      ...
      const std::vector<Situation> SITUATIONS = {
        ...
        {"the largest cash the block holds", 0xFFFFFFFFu, 7, 255, {'A', 'B', 'C', 'D', 'E', 'F', 'G', 13}, OTHER, LAVE, false},
        {"an empty name", 4242u, 3, 35, {13, 'X', 'X', 'X', 'X', 'X', 'X', 13}, LAVE, LAVE, false},
        {"a name with no room to spare", 99999u, 6, 1, {'Z', 'Z', 'Z', 'Z', 'Z', 'Z', 'Z', 13}, OTHER, OTHER, false},
      ...
      for (const Situation& situation : SITUATIONS)
      {
        ...
        for (const std::uint8_t caseFlags : {std::uint8_t{0x00}, std::uint8_t{0x80}})
        {
          ...
          {
            const std::wstring where = Widen(std::string("control code ") + static_cast<char>('0' + token) + " (" + situation.what +
            ...
            {
              cpu.memory[static_cast<std::uint16_t>(cashAt + index)] = static_cast<std::uint8_t>(situation.cash >> (24 - 8 * index));
            }
            ...
            StateTokens tokens(printer, text, commander, std::span<const std::uint8_t, Elite::COMMANDER_NAME_SIZE>(situation.name), current,
                               selected, situation.misJumped);
            printer.SetValueTokens(&tokens);
            printer.Print(token);

            // ---- compare -----------------------------------------------------------------------
            Assert::AreEqual(gameCharacters.size(), recording.characters.size(),
                             (where + L": how many characters were printed -- game \"" + Widen(Describe(gameCharacters)) + L"\", port \"" +
            ...
            for (std::size_t index = 0; index < gameCharacters.size(); ++index)
            {
              Assert::AreEqual(gameCharacters[index], recording.characters[index],
                               (where + L": character " + std::to_wstring(index) + L" -- game \"" + Widen(Describe(gameCharacters)) +
            ...
            Assert::AreEqual(cpu.memory[oracle.Label("QQ17")], printer.CaseFlags(), (where + L": the case flags afterwards").c_str());
      ...
      Logger::WriteMessage(
        ("state control codes: " + std::to_string(compared) + " compared character for character. Code 5 reads:\n  " + sample + "\n")
          .c_str());
    }
  };
