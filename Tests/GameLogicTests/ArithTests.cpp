...
namespace GameLogicTests
{
  ...
  public:
    ...
    TEST_METHOD(UnsignedMultiplyMatchesExhaustively)
    {
      ...
      for (std::uint32_t p = 0; p < 256; ++p)
      {
        ...
        {
          cpu.memory[zp.zeroPageP] = static_cast<std::uint8_t>(p);
          cpu.memory[zp.zeroPageQ] = static_cast<std::uint8_t>(q);
          cpu.a = cpu.x = cpu.y = 0;
          cpu.sp = 0xFD;
          cpu.c = false;
          ...
          Assert::AreEqual<std::uint32_t>(cpu.memory[zp.zeroPageP], product.low, Context(L"low byte", p, q).c_str());
    ...
    TEST_METHOD(MultiplyByRegisterMatchesExhaustively)
    {
      ...
      for (std::uint32_t p = 0; p < 256; ++p)
      {
        ...
        for (std::uint32_t x = 1; x < 256; ++x)
        {
          ...
          Assert::AreEqual<std::uint32_t>(cpu.memory[zp.zeroPageP], product.low, Context(L"low byte", p, x).c_str());
        }
      }
    }

    /// (A P) = Q * A for sign-magnitude operands, including the sign of the product -- and the
    /// exit carry, which is the last `ROR P`'s on the long path and the opening `LSR A`'s on
    /// `mu10`. No caller reads it; the model says what it is rather than guessing.
    TEST_METHOD(SignedMultiplyMatchesExhaustively)
    {
      ...
      for (std::uint32_t a = 0; a < 256; ++a)
      {
        for (std::uint32_t q = 0; q < 256; ++q)
        {
          ...
          Assert::AreEqual<std::uint32_t>(cpu.memory[zp.zeroPageP], product.low, Context(L"low byte", a, q).c_str());
          Assert::AreEqual(cpu.c, product.carry, Context(L"exit carry", a, q).c_str());
        }
      }
    }

    /// 6502: MULT12 -- the same multiplication, landing in (S R). The port has no routine for it
    /// since M2-b: a caller holds the `Product` and hands its pair on. What the sweep pins is that
    /// (S R) IS that pair, byte for byte.
    TEST_METHOD(SignedMultiplyIntoScratchMatchesExhaustively)
    {
      ...
      for (std::uint32_t a = 0; a < 256; ++a)
      {
        for (std::uint32_t q = 0; q < 256; ++q)
        {
          ...
          Assert::AreEqual<std::uint32_t>(cpu.memory[zp.zeroPageS], pair.hi, Context(L"S", a, q).c_str());
          Assert::AreEqual<std::uint32_t>(cpu.memory[zp.zeroPageR], pair.lo, Context(L"R", a, q).c_str());
        }
      }
    }
    ...
    TEST_METHOD(SquaringMatchesForEveryInput)
    {
      ...
      for (std::uint32_t value = 0; value < 256; ++value)
      {
        ...
        for (const Variant& variant : {Variant{"SQUA", true}, Variant{"SQUA2", false}})
        {
          ...
          const Product squared = variant.clearsSignBit ? Elite::Square(static_cast<std::uint8_t>(value))
  ...
  public:
    ...
    /// 6502: MU5 -- the block fill, which `MULT3` answers with for a zero multiplier. (`MU6`, the
    /// pair set, is `MULTS`'s zero exit and has no routine of its own in the port: nothing reads
    /// the second byte it clears.)
    TEST_METHOD(BlockFillMatchesForEveryInput)
    {
      if (OracleMissing())
      {
        return;
      ...
      for (std::uint32_t value = 0; value < 256; ++value)
      {
        ...
        const Elite::KBlock fill = Elite::KBlock::Filled(static_cast<std::uint8_t>(value));
        const std::uint8_t bytes[4] = {fill.low, fill.mid, fill.high, fill.top};
        for (std::uint16_t byte = 0; byte < 4; ++byte)
        {
          Assert::AreEqual<std::uint32_t>(cpu.memory[static_cast<std::uint16_t>(blockAddress + byte)], bytes[byte],
    ...
    TEST_METHOD(ScaledMultiplyMatchesExhaustively)
    {
      ...
      for (std::uint32_t a = 0; a < 256; ++a)
      {
        for (std::uint32_t p = 0; p < 256; ++p)
        {
          ...
          Assert::AreEqual<std::uint32_t>(cpu.memory[zp.zeroPageP], product.low, Context(L"low byte", a, p).c_str());
    ...
    TEST_METHOD(DivideBy96MatchesForEveryInput)
    {
      ...
      for (std::uint32_t a = 0; a < 256; ++a)
      {
        ...
        const std::uint8_t result = Elite::DivideBy96(static_cast<std::uint8_t>(a));
  ...
  public:
    ...
    TEST_METHOD(LogDivideMatchesExhaustively)
    {
      ...
      for (std::uint32_t a = 0; a < 256; ++a)
      {
        for (std::uint32_t q = 0; q < 256; ++q)
        {
          ...
          Assert::AreEqual<std::uint32_t>(cpu.memory[zp.zeroPageR], quotient.value, Context(L"R", a, q).c_str());
    ...
    TEST_METHOD(SineMultiplyMatchesExhaustively)
    {
      ...
      for (std::uint32_t a = 0; a < 256; ++a)
      {
        for (std::uint32_t kValue = 0; kValue < 256; ++kValue)
        {
          ...
          const Elite::LogProduct result = Elite::MultiplyBySine(static_cast<std::uint8_t>(kValue), static_cast<std::uint8_t>(a), carryIn);

          Assert::AreEqual<std::uint32_t>(cpu.a, result.value, Context(L"product", a, kValue).c_str());
    ...
    TEST_METHOD(RestoringDivideMatchesExhaustively)
    {
      ...
      for (std::uint32_t a = 0; a < 256; ++a)
      {
        for (std::uint32_t q = 0; q < 256; ++q)
        {
          ...
          Assert::AreEqual<std::uint32_t>(cpu.memory[zp.zeroPageP], result.whole, Context(L"quotient", a, q).c_str());
          Assert::AreEqual<std::uint32_t>(cpu.memory[zp.zeroPageR], result.fraction, Context(L"R", a, q).c_str());
    ...
    TEST_METHOD(SquareRootMatchesExhaustively)
    {
      ...
      for (std::uint32_t r = 0; r < 256; ++r)
      {
        for (std::uint32_t q = 0; q < 256; ++q)
        {
          cpu.memory[zp.zeroPageR] = static_cast<std::uint8_t>(r);
          cpu.memory[zp.zeroPageQ] = static_cast<std::uint8_t>(q);
          cpu.a = cpu.x = cpu.y = 0;
          cpu.sp = 0xFD;
          cpu.c = false;
          ...
          Assert::AreEqual<std::uint32_t>(cpu.memory[zp.zeroPageQ], root.value, Context(L"root", r, q).c_str());
