...
public:
  ...
  void AbortMissile(std::uint8_t _colour) override { aborts.push_back(_colour); }
  void ShowMessage(std::uint8_t _token) override { messages.push_back(_token); }
  void ToggleStationIndicator() override { ++stationBlobs; }
  void ResetMissileIndicators() override { ++missileBlobs; }
};
...
struct SpawnLabels
{
  ...
  explicit SpawnLabels(const OracleImage& _oracle)
  {
    ...
    many = _oracle.Label("MANY");   junk = _oracle.Label("JUNK");
    slsp = _oracle.Label("SLSP");   mstg = _oracle.Label("MSTG");
    sspr = _oracle.Label("SSPR");   tp = _oracle.Label("TP");
    tally = _oracle.Label("TALLY"); fist = _oracle.Label("FIST");
    tribble = _oracle.Label("TRIBBLE"); qq20 = _oracle.Label("QQ20");
    tek = _oracle.Label("tek");     qq15 = _oracle.Label("QQ15");
    inwk = _oracle.Label("INWK");   rand = _oracle.Label("RAND");
    lso = _oracle.Label("LSO");     xx4 = _oracle.Label("XX4");
    inf = _oracle.Label("INF");     xx0 = _oracle.Label("XX0");
  }
};

/// One bubble, put into both sides identically: the slot list, the blocks, the counts and the
/// line heap the blocks point into.
void SeedBubble(Cpu6502& _cpu, Elite::Bubble& _bubble, Elite::LineHeap& _heap,
                const SpawnLabels& _at, const std::vector<std::uint8_t>& _fleet)
{
  ...
  for (std::size_t slot = 0; slot < _fleet.size() && slot < Elite::MAX_SHIPS; ++slot)
  {
    ...
    for (std::size_t byte = 0; byte < Elite::SHIP_BLOCK_SIZE; ++byte)
    {
      ...
      _cpu.memory[static_cast<std::uint16_t>(
        _at.kPercent + slot * Elite::SHIP_BLOCK_SIZE + byte)] = value;
    }
    ...
    const std::uint8_t size =
      (blueprint == 0u) ? std::uint8_t{ 0 }
                        : Elite::ShipByte(static_cast<std::uint16_t>(blueprint + 5u));
    ...
    _cpu.memory[static_cast<std::uint16_t>(_at.kPercent + slot * Elite::SHIP_BLOCK_SIZE
                                           + Elite::SHIP_HEAP_LOW_OFFSET)] =
      static_cast<std::uint8_t>(heapAt);
    _cpu.memory[static_cast<std::uint16_t>(_at.kPercent + slot * Elite::SHIP_BLOCK_SIZE
                                           + Elite::SHIP_HEAP_HIGH_OFFSET)] =
...
void CompareBubble(const Cpu6502& _cpu, const Elite::Bubble& _bubble, const Elite::LineHeap& _heap,
                   const SpawnLabels& _at, const std::wstring& _where)
{
  for (std::size_t slot = 0; slot < _bubble.slots.size(); ++slot)
  {
    Assert::AreEqual(_cpu.memory[static_cast<std::uint16_t>(_at.frin + slot)],
                     _bubble.slots[slot], (_where + L": FRIN+" + std::to_wstring(slot)).c_str());
  }
  for (std::size_t slot = 0; slot < Elite::MAX_SHIPS; ++slot)
  {
    for (std::size_t byte = 0; byte < Elite::SHIP_BLOCK_SIZE; ++byte)
    {
      Assert::AreEqual(_cpu.memory[static_cast<std::uint16_t>(
                         _at.kPercent + slot * Elite::SHIP_BLOCK_SIZE + byte)],
                       _bubble.blocks[slot][byte],
                       (_where + L": K%+" + std::to_wstring(slot) + L"." + std::to_wstring(byte))
                         .c_str());
    }
  }
  for (std::size_t type = 0; type < _bubble.counts.size(); ++type)
  {
    Assert::AreEqual(_cpu.memory[static_cast<std::uint16_t>(_at.many + type)],
                     _bubble.counts[type], (_where + L": MANY+" + std::to_wstring(type)).c_str());
  }

  Assert::AreEqual(_cpu.memory[_at.junk], _bubble.junk, (_where + L": JUNK").c_str());
  Assert::AreEqual<std::uint32_t>(
    static_cast<std::uint32_t>(_cpu.memory[_at.slsp] | (_cpu.memory[_at.slsp + 1] << 8)),
   ...
   */
  constexpr std::uint16_t LINES_START = static_cast<std::uint16_t>(
...
public:
  ...
  TEST_METHOD(TheKillMatchesKILLSHP)
  {
        ...
        for (const std::uint8_t locked : { 0xFFu, 0u, 1u, 2u })
        {
          ...
          for (std::size_t slot = 0; slot < living; ++slot)
          {
            ...
            const std::uint8_t ai =
              static_cast<std::uint8_t>(0x80u | ((slot == 0u ? living - 1u : 0u) << 1));
          ...
          cpu.memory[static_cast<std::uint16_t>(at.xx0 + 1)] =
          ...
          Elite::KillShip(bubble, heap, state, work, commander, effects,
                          static_cast<std::uint8_t>(victim));

          const std::wstring where =
            Widen("KILLSHP fleet=" + std::to_string(fleet.size()) + " victim="
                  + std::to_string(victim) + " locked=" + std::to_string(locked));

          CompareBubble(cpu, bubble, heap, at, where);
          Assert::AreEqual(cpu.memory[at.tp], commander.At(Elite::Field::MissionProgress),
                           (where + L": TP").c_str());
          ...
          Assert::AreEqual<std::size_t>(cpu.trapHits.size(),
                                        effects.aborts.size() + effects.messages.size()
                                          + effects.stationBlobs,
    ...
    Logger::WriteMessage(("KILLSHP: " + std::to_string(compared) + " kills, "
                          + std::to_string(aborted) + " missiles unlocked, "
                          + std::to_string(stations) + " stations, " + std::to_string(missions)
                          + " missions, " + std::to_string(moved) + " relocations")
                           .c_str());
  }
};
...
public:
  ...
  TEST_METHOD(TheSystemMatchesSOLAR)
  {
    ...
    const std::vector<System> SYSTEMS = {
      { { 0x5A, 0x00, 0x48, 0x02, 0x53, 0xB7 }, 5, 0, 0, 0,
        { 0xC3, 0x71, 0x2B, 0x49 }, false, L"Lave, no Trumbles" },
      { { 0x5A, 0x01, 0x48, 0x05, 0x53, 0xB7 }, 8, 2, 0, 12,
        { 0xC3, 0x71, 0x2B, 0x49 }, false, L"a pair breeding" },
      { { 0x11, 0x02, 0x33, 0x07, 0x91, 0x44 }, 1, 200, 3, 255,
        { 0x01, 0x02, 0x03, 0x04 }, true, L"a swarm, and a fugitive" },
      { { 0xFF, 0x03, 0xFF, 0x00, 0xFF, 0xFF }, 14, 0xFF, 0x7F, 1,
        { 0xFE, 0xFF, 0xFD, 0xFC }, false, L"every seed byte high" },
      { { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 }, 0, 1, 0xFF, 0,
        { 0x80, 0x7F, 0x81, 0x00 }, true, L"every seed byte low" },
      { { 0x37, 0x02, 0x19, 0x04, 0x2A, 0x60 }, 3, 7, 1, 9,
        { 0x55, 0xAA, 0x33, 0xCC }, false, L"a generator that carries out" },
      { { 0x48, 0x01, 0x22, 0x06, 0x71, 0x13 }, 11, 33, 0, 200,
        { 0x0F, 0xF0, 0x77, 0x88 }, true, L"and one that does not" },
    };
    ...
    for (const System& system : SYSTEMS)
    {
      ...
      Assert::AreEqual(cpu.memory[at.tribble], commander.bytes[tribble],
                       (where + L": TRIBBLE").c_str());
      Assert::AreEqual(cpu.memory[static_cast<std::uint16_t>(at.tribble + 1)],
                       commander.bytes[tribble + 1u], (where + L": TRIBBLE+1").c_str());
      Assert::AreEqual(cpu.memory[at.fist], commander.At(Elite::Field::LegalStatus),
                       (where + L": FIST").c_str());
      for (std::size_t good = 0; good < 17u; ++good)
      {
