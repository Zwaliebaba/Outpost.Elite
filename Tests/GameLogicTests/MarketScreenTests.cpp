...
namespace GameLogicTests
{
  ...
  namespace
  {
    ...
    KeyboardRun RunWithKeys(Cpu6502& _cpu, std::uint16_t _routine, std::uint16_t _keyRead, const std::vector<std::uint8_t>& _keys,
                            std::uint16_t _leaveAt = 0, std::uint32_t _budget = 400'000, std::uint16_t _alsoLeaveAt = 0)
    {
  ...
  public:
    ...
    TEST_METHOD(ReadNumberMatchesTheShippedRoutine)
    {
      ...
      for (const Script& script : SCRIPTS)
      {
        ...
        {
        public:
          explicit Recording(std::vector<std::uint8_t>& _into) noexcept
            : m_into(_into)
          {
          }
          void Put(std::uint8_t _character) override
          {
            m_into.push_back(_character);
          }
         ...
         */
        Assert::AreEqual(run.leftEarly, entry.outcome == DigitResult::LeaveScreen, (where + L": whether the screen was abandoned").c_str());
        if (!run.leftEarly)
        {
          Assert::AreEqual(cpu.c, entry.outcome == DigitResult::TooBig, (where + L": the carry the caller branches on").c_str());
        }
         ...
         */
        Assert::IsTrue(entry.outcome != DigitResult::Accepted, (where + L": gnum never returns mid-number").c_str());
        ...
        {
          Assert::AreEqual(gameEchoed[index], ourEchoed[index], (where + L": echoed character " + std::to_wstring(index)).c_str());
        }
  ...
  public:
    ...
    TEST_METHOD(BuyingCargoMatchesTheShippedRoutine)
    {
      ...
      for (const Scenario& scenario : SCENARIOS)
      {
        ...
        const KeyboardRun run = RunWithKeys(cpu, oracle.Label("TT219"), tt217, scenario.keys, oracle.Label("BAY2"), 4'000'000);
      ...
      Logger::WriteMessage(
        ("TT219: " + std::to_string(compared) + " buy screens compared character for character, with the cursor stamped\n").c_str());
    }
  };
  ...
  public:
    ...
    TEST_METHOD(ListingAndSellingCargoMatchTheShippedRoutines)
    {
      ...
      for (const Scenario& scenario : SCENARIOS)
      {
        ...
        const std::wstring where = Widen(std::string(inventory ? "TT213: " : "TT210: ") + scenario.what);
        ...
        const KeyboardRun run = RunWithKeys(cpu, entry, tt217, scenario.keys, oracle.Label("BAY2"), 4'000'000);
        Assert::IsTrue(run.completed, (where + L": the shipped screen should finish").c_str());
      ...
      Logger::WriteMessage(
        ("TT210/TT213: " + std::to_string(compared) + " cargo listings compared character for character, with the cursor stamped\n")
          .c_str());
    }
  };
  ...
  public:
    ...
    TEST_METHOD(TheStatusReportMatchesTheShippedRoutine)
    {
      ...
      Logger::WriteMessage(
        ("STATUS: " + std::to_string(compared) + " status screens compared character for character, with the cursor stamped\n").c_str());
    }
  };
  ...
  public:
    ...
    TEST_METHOD(BuyingEquipmentMatchesTheShippedRoutine)
    {
      ...
      Logger::WriteMessage(
        ("EQSHP: " + std::to_string(compared) + " equipment screens compared character for character, with the cursor stamped\n").c_str());
    }
  };
