...
namespace GameLogicTests
{
  ...
  public:
    ...
    TEST_METHOD(TheDefaultCommanderMatchesTheShippedBlock)
    {
      ...
      {
        Assert::AreEqual<std::uint32_t>(cpu.memory[static_cast<std::uint16_t>(layout.na2 + index)], Elite::DEFAULT_COMMANDER[index],
    ...
    TEST_METHOD(ChecksumMatchesTheShippedRoutine)
    {
      ...
      for (const CommanderBlock& block : SampleBlocks())
      {
        ...
        Assert::AreEqual<std::uint32_t>(cpu.a, Elite::Checksum(block), (L"CHECK differs on block " + std::to_wstring(compared)).c_str());
        ++compared;
      }
    ...
    TEST_METHOD(SecondChecksumMatchesTheShippedRoutine)
    {
      ...
      for (const CommanderBlock& block : SampleBlocks())
      {
        ...
        Assert::AreEqual<std::uint32_t>(cpu.a, Elite::Checksum2(block), (L"CHECK2 differs on block " + std::to_wstring(compared)).c_str());
        ++compared;
      }
    ...
    TEST_METHOD(SavingACommanderMatchesTheShippedRoutine)
    {
      ...
      for (const CommanderBlock& block : SampleBlocks())
      {
        ...
        {
          cpu.memory[static_cast<std::uint16_t>(layout.block + step)] = cpu.memory[static_cast<std::uint16_t>(layout.tp + step)];
        }
        ...
        for (std::size_t index = 0; index < file.size(); ++index)
        {
          Assert::AreEqual<std::uint32_t>(
            ...
            (L"saved file byte " + std::to_wstring(index) + L" on block " + std::to_wstring(compared)).c_str());
        }
    ...
    TEST_METHOD(TheGameAndThePortAgreeOnWhichCommandersAreAcceptable)
    {
      ...
      for (const CommanderBlock& block : SampleBlocks())
      {
        ...
        for (const bool corrupt : {false, true})
        {
          ...
          Assert::AreEqual(ourAnswer, gameGotPast,
                           (std::wstring(L"the two disagree on a ") + (corrupt ? L"corrupted" : L"sound") + L" commander").c_str());
      ...
      Logger::WriteMessage(("DFAULT: " + std::to_string(accepted) + " commanders accepted, " + std::to_string(rejected) +
                            " rejected -- and the shipped routine hangs on exactly those")
                             .c_str());
