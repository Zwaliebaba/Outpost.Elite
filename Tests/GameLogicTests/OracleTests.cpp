  ...
  public:
    ...
    TEST_METHOD(OracleIsPresent)
    {
      ...
      Assert::IsTrue(oracle.Available(),
                     (L"the oracle is not loaded: " + Widen(oracle.Reason()) + L"\nRun: python tools/labels.py --assemble").c_str());
    }
    ...
    TEST_METHOD(TheWholeGameIsLoadedAndItsLabelsResolve)
    {
      ...
      Assert::AreEqual<std::uint32_t>(761u, nonBlank, L"the dashboard bitmap is loaded at DSTORE%, not blank");
      ...
      for (const char* name :
           {"DORND", "MULTU", "MULT1", "FMLTU", "ADD", "ARCTAN", "TIS1", "LL28", "MVEIT", "TACTICS", "SNE", "ACT", "QQ18", "XX21"})
      {
