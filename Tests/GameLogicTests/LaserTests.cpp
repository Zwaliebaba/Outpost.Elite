...
public:
  ...
  TEST_METHOD(FiringMatchesLASLI)
  {
        ...
        for (const bool carryIn : { false, true })
        {
          ...
          const std::wstring where =
            Widen("LASLI seed " + std::to_string(seed) + " view " + std::to_string(view)
                  + " carry " + std::to_string(carryIn ? 1 : 0));
          ...
          for (std::uint16_t offset = 0; offset < Elite::Canvas::SCREEN_SIZE; ++offset)
          {
            ...
            {
              Assert::Fail((where + L": screen differs at offset " + std::to_wstring(offset)
    ...
    Logger::WriteMessage(("LASLI: " + std::to_string(compared) + " shots, "
                          + std::to_string(heights.size()) + " convergence rows and "
