...
struct Labels
{
  ...
  explicit Labels(const OracleImage& _oracle)
  {
    ...
    screen = static_cast<std::uint16_t>(
      (cpu.memory[_oracle.Label("ylookupl")] | (cpu.memory[_oracle.Label("ylookuph")] << 8)) - 0x20);
  }
};
...
Marks CompareAndMeasure(const Cpu6502& _cpu, std::uint16_t _base, const Elite::Canvas& _canvas,
                        const std::wstring& _context)
{
  ...
  for (std::uint16_t offset = 0; offset < Elite::Canvas::SCREEN_SIZE; ++offset)
  {
    ...
    {
      ++marks.bytes;
      const int row = static_cast<int>((offset - 0x20u) / Elite::Canvas::ROW_BYTES) * 8
                      + static_cast<int>(offset & 7u);
...
void PlaceShip(Cpu6502& _cpu, const Labels& _at, Elite::ShipBlock& _ship,
               const std::array<std::uint8_t, 6>& _position, std::uint8_t _state,
               std::uint8_t _type) noexcept
{
  ...
  for (int which = 0; which < 6; ++which)
  {
    ...
    _cpu.memory[static_cast<std::uint16_t>(_at.inwk + OFFSETS[which])] =
      _position[static_cast<std::size_t>(which)];
  }
  _ship[31] = _state;
...
public:
  ...
  TEST_METHOD(TheBlipAndItsStickMatchSCAN)
  {
        ...
        for (std::uint32_t height = 0; height < 64; ++height)
        {
          for (const std::uint8_t heightSign : { std::uint8_t{ 0 }, std::uint8_t{ 0x80 } })
          {
            ...
            const std::array<std::uint8_t, 6> position = {
              0x15u, 0x00u, static_cast<std::uint8_t>(height), heightSign,
              static_cast<std::uint8_t>(depth), depthSign,
            };
            ...
            const std::wstring where =
              Widen("SCAN z=" + std::to_string(depth) + (depthSign ? "-" : "+") + " y="
                    + std::to_string(height) + (heightSign ? "-" : "+"));
    ...
    Logger::WriteMessage(("SCAN: " + std::to_string(compared) + " blips -- " + std::to_string(up)
                          + " up, " + std::to_string(down) + " down, " + std::to_string(dotOnly)
                          + " dot only; tallest " + std::to_string(tallest) + " rows")
                           .c_str());
  }
  ...
  TEST_METHOD(TheBlipAcrossTheScannerMatchesSCAN)
  {
    ...
    const Depth DEPTHS[] = {
      { 0u, 0u, 0u, 0u },       { 40u, 0u, 20u, 0u },     { 40u, 0x80u, 20u, 0u },
      { 63u, 0u, 63u, 0x80u },  { 63u, 0x80u, 63u, 0u },
    };
    ...
    for (std::uint32_t across = 0; across < 64; ++across)
    {
      for (const std::uint8_t sign : { std::uint8_t{ 0 }, std::uint8_t{ 0x80 } })
      {
        for (const Depth& item : DEPTHS)
        {
          ...
          const std::array<std::uint8_t, 6> position = { static_cast<std::uint8_t>(across),
                                                         sign,
                                                         item.height,
                                                         item.heightSign,
                                                         item.depth,
          ...
          const std::wstring where =
            Widen("SCAN x=" + std::to_string(across) + (sign ? "-" : "+") + " depth "
          ...
          wrapped += (draw.x1 == static_cast<std::uint8_t>(0xC0u & Elite::SCANNER_COLOUR_TABLE[11]))
                       ? 1u
                       : 0u;
          ++compared;
        }
    ...
    Assert::AreEqual<std::size_t>(4u, patterns.size(),
                                  L"all four of a colour's pixel patterns were reached");
...
public:
      ...
      for (const std::uint8_t y : HIGHS)
      {
        for (const std::uint8_t z : HIGHS)
        {
          ...
          for (int variant = 0; variant < 3; ++variant)
          {
            ...
            const std::wstring where = Widen("TAS2 highs " + std::to_string(x) + ","
  ...
  TEST_METHOD(TheCompassOffsetMatchesSPS2)
  {
    ...
    for (std::uint32_t value = 0; value < 256; ++value)
    {
      for (const bool carryIn : { false, true })
      {
        ...
        const std::wstring where = Widen("SPS2(" + std::to_string(value) + ", carry "
  ...
  TEST_METHOD(TheCompassDotMatchesDOT)
  {
    ...
    for (std::uint32_t x = 0; x < 256; ++x)
    {
      ...
      {
        for (const std::uint8_t colour :
             { Elite::COMPASS_AHEAD, Elite::COMPASS_BEHIND, std::uint8_t{ 0x55 } })
        {
          ClearScreen(cpu, at.screen);
          canvas.Clear();
          ...
          const std::wstring where = Widen("DOT(" + std::to_string(x) + ", " + std::to_string(y)
  ...
  TEST_METHOD(TheCompassMatchesSP2)
  {
    ...
    for (std::uint32_t across = 0; across < 256; ++across)
    {
      for (std::uint32_t down = 0; down < 256; ++down)
      {
        ...
        const std::wstring where =
          Widen("SP2(" + std::to_string(across) + ", " + std::to_string(down) + ")");
    ...
    for (std::uint32_t seed = 0; seed < 120; ++seed)
    {
      for (const std::uint8_t stations : { std::uint8_t{ 0 }, std::uint8_t{ 1 } })
      {
        ...
        for (int frame = 0; frame < 2; ++frame)
        {
          ...
          const std::wstring where =
            Widen("COMPAS seed " + std::to_string(seed) + (stations ? " station" : " planet")
