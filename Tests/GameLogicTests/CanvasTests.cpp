...
namespace GameLogicTests
{
  ...
  public:
    ...
    TEST_METHOD(ComputedCellOffsetsMatchTheGamesTable)
    {
      ...
      {
        const std::uint16_t table = static_cast<std::uint16_t>(Elite::CELL_ADDRESS_LOW[row] | (Elite::CELL_ADDRESS_HIGH[row] << 8));
    ...
    TEST_METHOD(LineMatchesTheShippedRoutineOverAWideSample)
    {
      ...
      Logger::WriteMessage(("LOIN wide: " + std::to_string(cases) + " lines, " + std::to_string(drawn) + " marked bytes\n").c_str());
    }
