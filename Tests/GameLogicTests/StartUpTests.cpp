...
{
public:
  explicit RecordingStart(std::vector<std::uint8_t> _answers) noexcept : m_answers(std::move(_answers)) {}

  void ResetUniverse() override { seams.push_back({ "RESET", 0, 0, 0 }); }
  void ResetShip() override { seams.push_back({ "RES2", 0, 0, 0 }); }
  void ClearKeyLogger() override { seams.push_back({ "ZEKTRAN", 0, 0, 0 }); }
  void StartTheme() override { seams.push_back({ "startat", 0, 0, 0 }); }
...
{
  void Put(std::uint8_t) override { ++characters; }
...
public:
  ...
  bool Read(std::span<const std::uint8_t, Elite::COMMANDER_NAME_SIZE>,
            std::span<std::uint8_t, Elite::COMMANDER_FILE_SIZE>) override
  {
    ++reads;
...
public:
    ...
    for (std::uint16_t x = 0; x < 256; x += 17)
    {
      for (std::uint16_t y = 0; y < 256; y += 13)
      {
        ...
        // 6502: jmp.
        {
          ...
          Elite::CurrentSystemToCrosshairs(commander, static_cast<std::uint8_t>(x),
                                           static_cast<std::uint8_t>(y));

          Assert::AreEqual(cpu.memory[qq0], commander.At(Elite::Field::SystemX), L"jmp: the current x");
          Assert::AreEqual(cpu.memory[static_cast<std::uint16_t>(qq0 + 1)],
                           commander.At(Elite::Field::SystemY), L"jmp: the y");
        }
  ...
  TEST_METHOD(TheDockingBayAndForcedKeysMatchTheShippedRoutines)
  {
    ...
    for (const Case& item : CASES)
    {
      ...
      const Elite::ForcedKey result =
        (item.entry == oracle.Label("BAY"))
          ? Elite::EnterDockingBay(portDocked, 0, 0, false)
          : Elite::ForceKey(item.key, portDocked, 0, 0, false);

      Assert::AreEqual(cpu.memory[qq12], portDocked, (where + L": the docked flag").c_str());
      Assert::AreEqual(static_cast<int>(docked ? Elite::MainLoop::Docked : Elite::MainLoop::InSpace),
                       static_cast<int>(result.loop), (where + L": which loop was entered").c_str());
  ...
  TEST_METHOD(TheTitleSequenceMatchesTheShippedRoutine)
  {
     ...
     */
    Assert::AreEqual<std::uint16_t>(static_cast<std::uint16_t>(oracle.Label("TT170") + 6),
                                    oracle.Label("DEATH2"), L"TT170 should fall into DEATH2");
    Assert::AreEqual<std::uint16_t>(static_cast<std::uint16_t>(oracle.Label("DEATH2") + 6),
                                    oracle.Label("BR1"), L"DEATH2 should fall into BR1");
    Assert::AreNotEqual<std::uint8_t>(0x60, oracle.Fresh().memory[static_cast<std::uint16_t>(
                                              oracle.Label("RES2") - 1)],
    ...
    const std::vector<Script> SCRIPTS = {
      ...
      { "Y, then leave the menu", false, Elite::KEY_YES_INTERNAL, { '5' },
        { ZEK, START, FIRST, STOP, START, BLOB, SECOND, STOP } },
      { "Y, toggle the media, then leave", false, Elite::KEY_YES_INTERNAL, { '3', '5' },
        { ZEK, START, FIRST, STOP, START, BLOB, SECOND, STOP } },
      { "a cold start", true, 'N', {}, { RESET, RES2, ZEK, START, FIRST, BLOB, SECOND, STOP } },
      { "a cold start into the menu", true, Elite::KEY_YES_INTERNAL, { '5' },
    ...
    for (const Script& script : SCRIPTS)
    {
      ...
      std::vector<std::pair<std::uint16_t, std::string>> named = {
        ...
        { oracle.Label("stopat"), "stopat" },   { oracle.Label("msblob"), "msblob" },
        { oracle.Label("RESET"), "RESET" },     { oracle.Label("RES2"), "RES2" },
      };
      ...
      for (const char* seam : { "DOXC", "DOYC", "MT9", "NLIN4", "FILEPR", "OTHERFILEPR", "KERNALSETUP",
                                "SETL1", "SWAPPZERO", "DELAY", "FLKB" })
      {
      ...
      DeviceStore store;
      Elite::SaveScreen save{ recursive, characters, extended, sink,    text,
                              keys,      lineEffects, store,    numbers };
      ...
      Elite::GameStart game{ effects,        save,     text,       commander,  name,       portImage,
                             buffer,         useDisk,  current,    selected,   crosshairX, crosshairY,
                             explosionCount, dockedFlag };

      const Elite::ForcedKey forced =
        script.coldStart ? Elite::ResetAndStartGame(game) : Elite::StartGame(game);
      ...
      Assert::AreEqual(script.expected.size(), effects.seams.size(),
                       (where + L": the port's seams are " + Describe(effects.seams) + L", expected "
                        + Describe(script.expected))
                         .c_str());
      Assert::AreEqual(seams.size(), effects.seams.size(),
                       (where + L": the game reached " + Describe(seams) + L", the port "
                        + Describe(effects.seams))
                         .c_str());
      for (std::size_t index = 0; index < seams.size(); ++index)
      {
        Assert::IsTrue(seams[index] == effects.seams[index] && seams[index] == script.expected[index],
                       (where + L": seam " + std::to_wstring(index) + L" -- game " + Describe(seams)
                        + L", port " + Describe(effects.seams))
                         .c_str());
      }
      ...
      Assert::AreEqual<std::uint8_t>(Elite::TITLE_PROMPT_COLUMN, firstColumn,
                                     (where + L": the column the game set").c_str());
      Assert::AreEqual<std::uint8_t>(Elite::TITLE_PROMPT_COLUMN, text.column,
                                     (where + L": the column the port set").c_str());
      ...
      {
        Assert::AreEqual(cpu.memory[static_cast<std::uint16_t>(oracle.Label("QQ2") + index)],
                         current.seeds.bytes[index], (where + L": QQ2 byte " + std::to_wstring(index)).c_str());
        Assert::AreEqual(cpu.memory[static_cast<std::uint16_t>(oracle.Label("QQ15") + index)],
                         selected.bytes[index], (where + L": QQ15 byte " + std::to_wstring(index)).c_str());
      }

      // DFAULT ran, so the live commander is what the image held.
      for (std::size_t index = 0; index + 1 < Elite::COMMANDER_BLOCK_SIZE; ++index)
      {
        Assert::AreEqual(cpu.memory[static_cast<std::uint16_t>(oracle.Label("TP") + index)],
                         commander.bytes[index],
      ...
      Assert::AreEqual(static_cast<int>(Elite::KeyAction::StatusMode),
                       static_cast<int>(forced.outcome.action), (where + L": what BAY's key reaches").c_str());
