  ...
  public:
    ...
    TEST_METHOD(NumbersMatchTheShippedRoutine)
    {
      ...
      const std::uint32_t values[] = {0u,        1u,        9u,         10u,        99u,         100u,       999u,    1000u,
                                      ...
                                      16777215u, 99999999u, 100000000u, 999999999u, 1000000000u, 4294967295u};
    ...
    TEST_METHOD(EntryPointsMatchTheShippedRoutines)
    {
      ...
      for (std::uint32_t value = 0; value < 65536u; value += 337u)
      {
        ...
        Assert::IsTrue(
          expected == collector.characters,
          Widen("TT11 " + std::to_string(value) + ": game " + Show(expected) + ", port " + Show(collector.characters)).c_str());
      }
      ...
      for (std::uint32_t value = 0; value < 256u; ++value)
      {
        ...
        Assert::IsTrue(expected == collector.characters,
                       Widen("pr2 " + std::to_string(value) + ": game " + Show(expected) + ", port " + Show(collector.characters)).c_str());
      }
    }
  };
