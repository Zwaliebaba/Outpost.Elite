...
namespace GameLogicTests
{
  ...
  public:
    ...
    TEST_METHOD(ProjectingAShipMatchesPROJ)
    {
                ...
                for (const std::uint8_t zLow : LOWS)
                {
                  for (const std::uint8_t zHigh : Z_HIGHS)
                  {
                    for (const std::uint8_t zSign : SIGNS)
                    {
                      ...
                      const std::uint8_t block[9] = {xLow, xHigh, xSign, xLow, yHigh, ySign, zLow, zHigh, zSign};
                      for (int byte = 0; byte < 9; ++byte)
                      {
                        cpu.memory[static_cast<std::uint16_t>(inwk + byte)] = block[byte];
                        PokeShip(ship, static_cast<std::size_t>(byte), block[byte]);
                      }
                      ...
                      const std::wstring where =
                        Widen("PROJ(x=" + std::to_string(xLow) + "/" + std::to_string(xHigh) + "/" + std::to_string(xSign) +
                              ", y=" + std::to_string(xLow) + "/" + std::to_string(yHigh) + "/" + std::to_string(ySign) +
                              ", z=" + std::to_string(zLow) + "/" + std::to_string(zHigh) + "/" + std::to_string(zSign) + ")");
      ...
      Assert::AreEqual<std::uint32_t>(2u * 4u * 2u * 4u * 2u * 2u * 3u * 2u, compared, L"the whole sweep ran");
  ...
  public:
    ...
    TEST_METHOD(DrawingADistantShipMatchesSHPPT)
    {
      ...
      for (const Position& position : POSITIONS)
      {
        ...
        const bool nowDrawn = Elite::Has(ship.state, Elite::ShipStateBit::OnScreen);
        ...
        if (!nowDrawn && (screen.x != before.x || screen.x1 != before.x1) && screen.y == before.y && screen.y1 == before.y1)
        {
          ++halfWritten;
        }
      }
  ...
  public:
    ...
    TEST_METHOD(TheSlopeStepsMatchLL120AndLL123)
    {
      ...
      Assert::AreEqual<std::uint32_t>(8u * 2u * 2u * 8u * 8u * 4u * 3u, compared, L"the whole sweep ran");
    ...
    TEST_METHOD(MovingAPointOnScreenMatchesLL118)
    {
      ...
      Assert::AreEqual<std::uint32_t>(4u * 4u * 4u * 4u * 4u * 2u * 2u, compared, L"the whole sweep ran");
  ...
  public:
    ...
    TEST_METHOD(ClippingALineMatchesLL145AndLL147)
    {
      ...
      const std::vector<std::uint16_t> COORDINATES = {0x0000, 0x0001, 0x008F, 0x0090, 0x00FF, 0x0100, 0xFF80, 0xFFFF};
            ...
            for (const std::uint16_t y2 : COORDINATES)
            {
              for (int entry = 0; entry < 2; ++entry)
              {
                ...
                const std::uint8_t block[6] = {
                  static_cast<std::uint8_t>(x1),      static_cast<std::uint8_t>(x1 >> 8), static_cast<std::uint8_t>(y1),
                  static_cast<std::uint8_t>(y1 >> 8), static_cast<std::uint8_t>(x2),      static_cast<std::uint8_t>(x2 >> 8),
                };
                for (std::size_t byte = 0; byte < 6u; ++byte)
                {
                ...
                const std::wstring where =
                  Widen(std::string(entry == 0 ? "LL145" : "LL147") + "(x1=" + std::to_string(x1) + ", y1=" + std::to_string(y1) +
                        ", x2=" + std::to_string(x2) + ", y2=" + std::to_string(y2) + ", dontclip=" + std::to_string(off) + ")");
  ...
  public:
    ...
    TEST_METHOD(DrawingAShipMatchesLL9)
    {
      ...
      Logger::WriteMessage(("LL9: " + std::to_string(compared) + " ships drawn -- " + std::to_string(drawn) + " as wireframes, " +
                            std::to_string(asPoints) + " as dots, " + std::to_string(erased) + " rubbed out, " +
                            std::to_string(heapFilled) + " filling the blueprint's heap")
                             .c_str());

      Assert::AreEqual<std::uint32_t>(static_cast<std::uint32_t>(Elite::SHIP_TYPE_COUNT) * static_cast<std::uint32_t>(PLACEMENTS.size()) *
