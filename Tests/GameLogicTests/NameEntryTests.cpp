...
{
public:
  void WaitFrames(std::uint8_t _frames) override { log.push_back(static_cast<std::uint32_t>(0x100u + _frames)); }
  void FlushKeyboard() override { log.push_back(0x200u); }
...
KeyboardRun RunWithKeys(Cpu6502& _cpu, std::uint16_t _routine, std::uint16_t _keyRead,
                        const std::vector<std::uint8_t>& _keys, std::uint32_t _budget = 200'000)
{
...
public:
  ...
  TEST_METHOD(ReadingALineMatchesTheShippedRoutine)
  {
    ...
    for (const Script& script : SCRIPTS)
    {
      ...
      Assert::AreEqual(expected.size(), sink.stamped.size(),
                       (where + L": how many characters were printed -- game \"" + Widen(Describe(expected))
                        + L"\", port \"" + Widen(Describe(sink.stamped)) + L"\"")
                         .c_str());
      for (std::size_t index = 0; index < expected.size(); ++index)
      {
        Assert::AreEqual(expected[index], sink.stamped[index],
                         (where + L": character " + std::to_wstring(index) + L" -- game \""
                          + Widen(Describe(expected)) + L"\", port \"" + Widen(Describe(sink.stamped)) + L"\"")
                           .c_str());
      }
  ...
  TEST_METHOD(TheNameRoutinesMatchTheShippedRoutines)
  {
    ...
    static constexpr std::array<std::uint8_t, Elite::COMMANDER_NAME_SIZE> EXISTING = { 'J', 'A', 'M', 'E',
                                                                                       'S', 'O', 'N', 13 };
    ...
    for (const Case& item : CASES)
    {
      ...
      const LineResult result =
        Elite::AskCommanderName(keys, sink, text, extended, effects, buffer, EXISTING, limits);

      Assert::IsFalse(keys.Overran(), (where + L": the port asked for more keys than the script holds").c_str());
      Assert::AreEqual(run.keysTaken, keys.Taken(), (where + L": how many keys were read").c_str());

      Assert::AreEqual(expected.size(), sink.stamped.size(),
                       (where + L": how many characters -- game \"" + Widen(Describe(expected)) + L"\", port \""
