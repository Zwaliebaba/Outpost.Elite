...
| `check_all.py` | Runs the <!--count:checks-->sixteen repository checks in CI's order, so a local run cannot leave one out. Thirteen CHECKS from ten scripts — `inventory.py` contributes `--check-includes` and `--strict`, and `check_gamelogic.py` and `check_outpost.py` each their own `--self-test`; `labels.py`, `golden_diff.py`, `bitmaps.py`, `extract_tables.py` and `mutate.py` are tools rather than checks and are not in it | **Built** (plan §6.127) |
