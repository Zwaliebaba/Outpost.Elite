...
| `check_all.py` | Runs the <!--count:checks-->twelve repository checks in CI's order, so a local run cannot leave one out. Twelve CHECKS from ten scripts — `inventory.py` contributes `--check-includes` and `--strict`, and `check_gamelogic.py` its own `--self-test`; `labels.py`, `golden_diff.py` and `mutate.py` are tools rather than checks and are not in it | **Built** (plan §6.127) |
