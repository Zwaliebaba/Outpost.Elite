...
| `check_all.py` | Runs the <!--count:checks-->thirteen repository checks in CI's order, so a local run cannot leave one out. `check_gamelogic.py`, `check_outpost.py`, `check_modernize.py` and `check_twins.py` each contribute their own `--self-test`; `bitmaps.py` is a tool rather than a check and is not in it; `mutate.py --check` is in it and `mutate.py --check-selftests` runs on CI's Ubuntu leg instead, because it needs a compiler. Four `inventory.py` entries went at M6-e with the ledger, `labels.py --check` at M6-b-7 with the label header it checked, and `c64_source.py --check-all` at M6-f with `Upstream/` | **Built** (plan §6.127) |
