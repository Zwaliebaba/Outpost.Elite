...
| `check_all.py` | Runs the <!--count:checks-->thirteen repository checks in CI's order, so a local run cannot leave one out. Thirteen CHECKS from eleven scripts — `inventory.py` contributes `--check-includes` and `--strict`, and `check_gamelogic.py` its own `--self-test`; `labels.py`, `golden_diff.py` and `mutate.py` are tools rather than checks and are not in it | **Built** (plan §6.127) |
