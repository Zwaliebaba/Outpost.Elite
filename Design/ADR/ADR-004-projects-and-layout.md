...
| `check_outpost.py` | Every `Elite::` name the executable calls still exists, with the right arity, and every member it names on an `Elite::`-typed variable exists on that type — the one check that reaches `Outpost/`, which the portable runner does not compile; `--self-test` plants a member that cannot resolve | **Built**; the member half added for Modernize.md M3; in CI |
...
| `bitmaps.py` | The three data tables that are pictures -- `FONT_DATA`, `DASHBOARD_IMAGE` and `SPRITE_DEFINITIONS` -- exported to indexed BMP sheets a paint program can edit, and imported back into the byte literals of their generated `.cpp` in place, by array name; a pixel's palette index is its bit value, an imported change trips `extract_tables.py --check` and `TableTests` by design, and `--self-test` proves the round trip is the identity | **Built**; a tool, not a check |
| `check_all.py` | Runs the <!--count:checks-->fourteen repository checks in CI's order, so a local run cannot leave one out. Fourteen CHECKS from eleven scripts — `inventory.py` contributes `--check-includes` and `--strict`, and `check_gamelogic.py` and `check_outpost.py` each their own `--self-test`; `labels.py`, `golden_diff.py`, `bitmaps.py` and `mutate.py` are tools rather than checks and are not in it | **Built** (plan §6.127) |
...
proven-green baseline, so a tally is a command (Risk R13). That fixes every tally FROM HERE: a
slice adds its mutants when it lands and anyone can re-run them. It does not fix the tallies
...
2. **CI latency, and some cost.** Measured on the same commit: the Ubuntu leg finishes in **34
   seconds** end to end (52s on a cold BeebAsm cache) against the Windows job's **4m35s**. The
   cost saving is real but smaller than it looks — GitHub charges roughly twice the per-minute
   rate for a Windows runner, so it is that factor times the shorter run, not the order of
   magnitude the rate alone suggests.
