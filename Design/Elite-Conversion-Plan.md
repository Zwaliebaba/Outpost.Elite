...
**Phase 1 is closed** but for **1c-c-b, the extended-token control codes**, which is scoped and
not built — nine of its twenty codes wait on `TT66`, `NLIN4` and phase-2 mission state. Every
other slice of phase 1 is built and green. §6.11 records what the drawing slices cost.
...
| **1c-c-b Control codes** | `MT1`–`MT29` behind the control-code seam, `vowel`, `whitetext`, `feed`, the `DTW1`–`DTW8` justification behaviour. | **Scoped 2026-09-03, not built.** `JMTB` was read out of the binary, so the twenty codes are now named rather than guessed at. Eleven are pure state — two or three stores to `DTW1`–`DTW8` or `QQ17`, or a cursor move — and land in one sitting on machinery that now exists. **Nine wait on routines phase 1 does not own**: codes 9 and 11 need `TT66` and `NLIN4` (the full screen clear with its border, and the title underline); 16 is reached through a self-modifying operand that 27 and 28 patch; 17, 18 and the mission codes reach `DETOK` with per-mission state phase 2 owns. Building the eleven alone would turn a seam that *counts* what it cannot do into one that silently ignores it, which is the trade this slice should not make on its own. |
