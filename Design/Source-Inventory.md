...
| `tt68`, `tt73`, `csh`, `plf`, `spc`, `tal`, `tt162`, `tt160`, `tt161`, `tt16a`, `tt163` | 11 | `Tokens.cpp`, `Screens.cpp` | Port. Each prints a value or moves the cursor, so they land with the commander and canvas work they depend on. `tal` — the galaxy number, which is `GCNT + 1` — is the one slice 2b needs: the long-range chart's title ends with it, so the chart reaches it through the value-token seam the printer already has. |
