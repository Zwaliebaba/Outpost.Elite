...
| ✅ `tkn2`, `jmtb` | 2 | `ExtendedTokenTables.cpp`, `ExtendedTokens.cpp` | **Extracted and ported 2026-09-03** (slices 1c-b and 1c-c-b). `JMTB` becomes a switch rather than a table of addresses, and every one of its twenty-one entries is compared against the shipped dispatch in two case states — the output, all seven `DTW` bytes, `QQ17` and 128 bytes of the line buffer. That matters more than it sounds: the game indexes the table from one, reading pairs of bytes out of the two addresses **before** `JMTB`, so an off-by-one sends every code to its neighbour's routine and most of the neighbours are plausible. |
