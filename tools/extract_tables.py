...
IT NO LONGER CHECKS. Until 2026-09-07 `--check` re-derived every table in CI and `TableTests`
compared each against the oracle's image; both were retired when the tables were declared the
port's own data. Three of them are pictures that `tools/bitmaps.py` edits through a BMP, and a
regeneration would silently put the original artwork back, so those three files are left alone
unless `--pictures` says otherwise.

    python tools/extract_tables.py             # regenerate the .cpp files, pictures excepted
    python tools/extract_tables.py --pictures  # the pictures too, back to the original artwork
...
# The files tools/bitmaps.py edits. The extractor still knows their tables -- the sizes and the
# addresses are documentation -- but a regeneration would overwrite imported artwork with the
# original's, so it leaves these alone unless told `--pictures`.
PICTURES = {"Font.cpp", "DashboardImage.cpp", "SpriteData.cpp"}
...
def main() -> int:
    ...
    parser.add_argument("--pictures", action="store_true",
                        help="rewrite the three picture files too, discarding whatever tools/bitmaps.py imported")
    ...
    for filename, tables in grouped.items():
        ...
        if filename in PICTURES and not args.pictures:
            print(f"kept   {path.relative_to(REPO)}  (a picture; --pictures to overwrite it)")
            continue
