...
AN IMPORTED CHANGE IS THE PORT'S OWN, AND THERE IS NO WAY BACK BUT VERSION CONTROL. These three
files were generated by `tools/extract_tables.py` from the assembled game, and until 2026-09-07
`--check` in CI and `TableTests` compared them against the oracle; both were retired, and the
extractor itself went with the original at M6-f. Nothing regenerates them and nothing guards them:
what the import writes is what the game draws.
...
def do_import(_name: str, _source: Path) -> int:
    ...
    if bitmap.game_colours:
        ...
        # `extract_tables.py --pictures` used to put the original artwork back and went with the
        # original at M6-f. Version control is the only way back for these three too now.
        print(f"NOTE   {bitmap.identifier} now differs from the byte the extractor left; keep the BMP")
    return 0
