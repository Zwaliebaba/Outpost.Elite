...
namespace Elite
{
  namespace
  {
    ...
    /// 6502: `JSTY`'s centre -- the middle of a control's range, and the sign both rate bytes
    /// carry when the ship is not turning.
    inline constexpr std::uint8_t CONTROL_CENTRE = 128;

    /// 6502: the roll and the drift a launch starts with, from ONE load into three bytes.
    inline constexpr std::uint8_t LAUNCH_ROLL = 3;

    /// 6502: `Yx2M1`, the bottom row `CHKON` counts as on screen, which `TT23` moves to 199 and
    /// this puts back.
    inline constexpr std::uint8_t SPACE_VIEW_LAST_ROW = 143;

    /// 6502: the `QQ11` value `TT110` leaves behind, which is not a view at all: it is what makes
  ...
  void ResetShipAndBubble(Universe& _universe, Ports& _ports) noexcept
  {
    ...
    /*
     * 6502: a bomb that is running is switched off, and then `BOMB` is stored to.
     *
    ...
    _universe.dust.count = STARDUST_COUNT; // 6502: NOST into NOSTM

    // 6502: 255 into entry 0 of both halves of the ball heap, and into the missile lock.
    _universe.heaps.ball[0] = 0xFFu;
    _universe.heaps.ball[BALL_HEAP_SIZE] = 0xFFu;
    _universe.bubble.missileTarget = 0xFFu;

    /*
     * 6502: 128 into `JSTY` and the two sign bytes, then doubled to zero for the six after them.
     *
    ...
    // 6502: 128 doubled is zero, which is where the next six stores get their value.
    ...
    // 6502: one load into `DELTA`, `ALPHA` and `ALP1` -- three meanings.
    _universe.flight.speed = LAUNCH_ROLL;
    _universe.flight.rollRate = LAUNCH_ROLL;
    _universe.flight.rollMagnitude = LAUNCH_ROLL;

    _universe.text.palette = TEXT_COLOUR_WHITE; // 6502: &10 into COL2
    _universe.clip.clippingOff = 0u;                  // 6502: zero into dontclip
    _universe.heaps.lowestVisibleRow = SPACE_VIEW_LAST_ROW;   // 6502: the bottom row into Yx2M1

    // 6502: the station bulb is a TOGGLE, so this puts it out only because `SSPR` says it was lit,
    ...
    // 6502: an ECM still counting down is switched off first.
    if (_universe.status.ecmCountdown != 0u)
    {
      StopEcm(_universe.canvas, _universe.status, _universe.sound, &_universe.picture);
    }

    // 6502: yu -- rub every ship off the screen and forget both line heaps.
    ClearAllShips(_universe.canvas, _universe.heaps, _universe.bubble, _universe.work, _universe.flight, _universe.view,
                  &_universe.picture);

    ClearBubbleState(_universe, _ports); // 6502: ZERO

    // 6502: `SLSP` back to the top of the heap block -- the heap is empty again.
  ...
  {
    ClearBubbleState(_universe, _ports); // 6502: ZERO, which leaves A at zero for the loop below

    /*
     * 6502: SAL3 -- a store loop from index 6 down to 0.
     *
    ...
    /*
     * 6502: REL5 -- the index moved into `QQ12`, then three more bytes from `FSH` filled.
     *
  ...
  {
    // 6502: LAUN -- the whoosh, and the carry `NOISE` returns is dropped because the next
    ...
    // 6502: the step is loaded here and `HFS2`'s first instruction is what stores it. This is the
    ...
    /*
     * 6502: HFS2 -- the view saved on the stack across the clear and restored after it.
     *
  ...
  {
    // 6502: HFS2 stores the step it was entered with -- the only writer on either tunnel's path,
    // which is the other half of §6.94's answer.
    _universe.heaps.circleStep = _step;

    /*
     * 6502: the view saved on the stack across the clear and restored after it.
     *
     * The screen is cleared to a space view and the view type is then PUT BACK to whatever the
     * caller had. So the tunnel is drawn on a blank space view while the game still believes it is
     * showing the docked screen -- which is exactly right, because the caller has not finished
  ...
  {
    /*
     * 6502: HYPNOISE -- the first hyperspace effect through `NOISE2` with a sustain and a
    ...
    // 6502: one vertical sync of `DELAY`, which is what the pacing object holds for.
    _ports.present.Present();

    (void)PlaySoundEffect(_universe.sound, SoundEffect::HyperspaceAgain, false);

    // 6502: into HFS2 with a step of 4, and straight back out.
  ...
  {
    // 6502: a zero `QQ12` skips to NLUNCH -- pressing "1" in flight only changes the view.
    if (_universe.dockedFlag != 0u)
    {
      // 6502: LAUN, over the docked screen it is still showing.
      DrawLaunchTunnel(_universe, _ports);
      ResetShipAndBubble(_universe, _ports); // 6502: RES2

      /*
       * 6502: TT111 -- for the SEEDS, not for the distance. The planet's look comes from the
      ...
      /*
       * 6502: the planet spawned, then the z sign and high byte moved and the station spawned.
       *
      ...
      (void)AddStation(_universe, _ports); // 6502: NWSPS

      _universe.flight.speed = LAUNCH_SPEED; // 6502: 12 into DELTA

      // 6502: the contraband penalty is ORed into `FIST` -- the fine is levied by leaving, not by
      // being scanned.
      ...
      _universe.view = VIEW_LAUNCHING; // 6502: 255 into QQ11

      /*
       * 6502: HFS1 -- eight rings over the screen the tunnel left, and they erase themselves
    ...
    // 6502: NLUNCH -- and the zero that clears `QQ12` is the same register the view change is
  ...
  {
    // 6502: the distance and the token are arguments here, stored and stacked; `TYPE` is a real
    // byte and `NWSHP` below reads it back.
    _universe.flight.type = _shipType;

    /*
     * 6502: `MULIE` set, `RESET` called, `MULIE` cleared.
     *
    ...
    _universe.keys.fill(0u); // 6502: ZEKTRAN

    // 6502: DOVDU19 with 32 -- the title screen's palette on the Master, a bare return here.

    SetUpScreen(_universe, _ports, TITLE_CLEAR_VIEW); // 6502: TT66 with 13
    _universe.view = 0u;                              // 6502: zero into QQ11

    /*
     * 6502: 96 into two bytes of the block, then 127 into the two counters.
     *
    ...
    // 6502: the 127 above stepped once and stored into QQ17 -- 128, which is sentence case, and
    // it is what the prompt prints in.
    _universe.text.caseFlags = 0x80u;

    // 6502: NWSHP, on the type stored above. The slot is kept because `LL9` needs the ship's block
    ...
    _universe.text.column = 6u;                            // 6502: DOXC with 6
    PrintThenNewline(_ports.printer, TITLE_HEADING_TOKEN); // 6502: plf with 30
    _ports.sink.Put(10u);                                  // 6502: TT26 with 10
    _universe.text.column = 6u;                            // 6502: DOXC with 6

    // 6502: `PATG` gates it -- the credits, and the byte that shows them also changes what the
    ...
    /*
     * 6502: a zero `brkd` branches straight to BRBR2 -- and the branch it guards is REPLACED
     * rather than omitted.
     *
    ...
    // 6502: BRBR2 -- zero into both.
    _universe.flight.speed = 0u;
    _universe.options.joystick = 0u;

    _universe.text.row = TITLE_PROMPT_ROW;     // 6502: 15 into YC
    _universe.text.column = TITLE_PROMPT_LEFT; // 6502: 1 into XC
    _ports.tokens.Print(_token);               // 6502: DETOK on the token pulled back off the stack -- the caller's own

    _universe.text.column = 3u;              // 6502: DOXC with 3
    _ports.tokens.Print(TITLE_BYLINE_TOKEN); // 6502: DETOK with 12

    _universe.flight.steerCone = TITLE_CNT2;       // 6502: 12 into CNT2
    _universe.flight.mainLoopCounter = TITLE_MCNT; // 6502: 5 into MCNT
    _universe.options.joystick = 0xFFu;            // 6502: &FF into JSTK

    for (;;)
    {
      // 6502: TLL2 -- the z high byte counts down to 1 and stops: the ship closes, then holds.
      ...
      /*
       * 6502: TL1 -- MVEIT, and nothing else.
       *
      ...
      /*
       * 6502: three stores after the move, with a dead load between two of them.
       *
      ...
      // 6502: LL9 -- the title's ship is never killed, so the carry it is reached with goes unread.
      DrawShip(_universe, _universe.bubble.blocks[slot], false);

      // 6502: RDKEY, and then the counter steps down.
      ...
      /*
       * 6502: the fire key tested first, and the loop's two other exits after it.
       *
  ...
  {
    // 6502: EXNO3 -- the explosion effect through `NOISE`, and the carry is whatever killed us.
    (void)PlaySoundEffect(_universe.sound, SoundEffect::Explosion, false);

    ResetShipAndBubble(_universe, _ports); // 6502: RES2

    // 6502: the speed is shifted LEFT twice, and the upstream comment says "divide by 4", which
    ...
    // 6502: BOX -- the SAME border again, and `BOX2` EORs, so drawing it twice rubs it out.
    DrawFullBorder(_universe.canvas, &_universe.picture);

    // 6502: two screen bytes zeroed -- the two `BOX` STORES instead of
    ...
    // 6502: nWq -- a whole new stardust field over the cleared screen.
    SeedStardustField(_universe.canvas, _universe.dust, _universe.rng, false, &_universe.picture);

    // 6502: DOYC then DOXC, both with 12 -- the cursor, then the sign.
    _universe.text.row = GAME_OVER_ROW;
    _universe.text.column = GAME_OVER_COLUMN;
    _ports.printer.PrintPhrase(GAME_OVER_TOKEN); // 6502: ex with 146
    ...
    do
    {
      const RngResult roll = SeedDebris(_universe.work, _universe.rng, carry); // 6502: Ze

      _universe.work.x.lo = static_cast<std::uint8_t>(roll.value >> 2); // 6502: two shifts right, then stored into INWK

      // 6502: zero into the view and four bytes of the block.
      _universe.view = 0u;
      ...
      // 6502: that zero stepped down into MCNT -- 255, so every timer-based call in the loop stops.
      _universe.flight.mainLoopCounter = 0xFFu;

      // 6502: the byte folded twice, into the y low byte and then the z low byte.
      ...
      // 6502: the random X masked into the roll counter -- a gentle roll, sign kept.
      _universe.work.rollCounter = static_cast<std::uint8_t>(roll.previous & 0x8Fu);

      _universe.status.laserCount = DEATH_FRAMES; // 6502: 64 into LASCT

      // 6502: a set carry rotated in from the top, masked, into the pitch counter -- and what is
      ...
      /*
       * 6502: a byte read out of the blueprint table decides plate or canister.
       *
      ...
      // 6502: fq1 -- and the carry it rotates in is the one the branch above just tested.
      ...
      // 6502: a random top bit into the slot's state byte -- half the wreckage is already dead,
  ...
  void Die(Universe& _universe, Ports& _ports) noexcept
  {
    ...
    ClearFlightKeys(_universe.keys); // 6502: U%

    // 6502: into DELTA -- and A is the zero `U%` left in it, so we stop dead.
    _universe.flight.speed = 0u;

    /*
     * 6502: one frame, the sprites hidden, then D2 -- a frame a turn until `LASCT` runs out.
     *
    ...
    // 6502: DET1 with 31, then DEATH2 -- the first is a bare return and the second is the
  ...
  void AbandonShip(Universe& _universe, Ports& _ports) noexcept
  {
    ...
    /*
     * 6502: FRS1 on a Cobra, and on the pirate blueprint if the first refused.
     *
    ...
    /*
     * 6502: ES1 -- 8 into the speed, 194 into the pitch, and half of that into the AI byte.
     *
    ...
    // 6502: ESL1 -- move, draw, step the counter down, round again. The death path `MVEIT` can
    ...
    while (_universe.work.ai != 0u)
    {
      ...
      /*
       * 6502: LL9 -- and the SLOT it writes back to is the one `FRS1` just filled, through
    ...
    // 6502: SCAN -- and it is drawn ONCE, after the loop, so the blip the animation left
    ...
    // 6502: ESL2 -- the cargo hold zeroed, SEVENTEEN bytes, because the loop runs from 16 down
    // THROUGH zero.
    ...
    _universe.commander.legalStatus = 0u; // 6502: FIST -- a clean record
    _universe.commander.escapePod = 0u;   // 6502: ESCP -- and the pod is spent

    /*
     * 6502: a population of zero skips to nosurviv; otherwise three random bits with the bottom
    ...
    {
      /*
       * 6502: the generator rotates in a SET carry, which `SCAN` left. Nothing between the two
    ...
    // 6502: nosurviv -- 70 into `QQ14`, seven light years, and the docking is the caller's, the
