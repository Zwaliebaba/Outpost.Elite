...
public:
  ...
  [[nodiscard]] RngResult NextRepeatable() noexcept { return Next(false); }

  [[nodiscard]] const std::array<std::uint8_t, 4>& State() const noexcept { return m_state; }
  void SetState(const std::array<std::uint8_t, 4>& _state) noexcept { m_state = _state; }
