...
namespace Elite
{
  ...
  NoiseResult PlaySoundEffect(SoundBuffer& _buffer, std::uint8_t _effect, bool _carryIn) noexcept
  {
    // 6502: NOISE -- CLV, then the routine.
    return MakeNoise(_buffer, _effect, _carryIn, false, 0u, 0u);
  }

  NoiseResult PlaySoundEffectPitched(SoundBuffer& _buffer, std::uint8_t _effect, std::uint8_t _sustain, std::uint8_t _frequency,
