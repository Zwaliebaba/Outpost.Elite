...
namespace Elite
{
  ...
  /*
   * abraxas, caravanserai and DFLAG -- what the screen is currently set up as.
   *
  ...
  struct ScreenState
  {
    std::uint8_t colourBank = 0x81;  ///< abraxas
    std::uint8_t bitmapMode = 0xC0;  ///< The LOWER half of the screen
    std::uint8_t dashboardShown = 0;

    /// `caravanserai`'s twin for the upper half, and the energy bomb's whole
    ...
    /*
     * The border colour the raster handler cycles while the bomb burns.
     *
    ...
    /*
     * HFX -- AND IT DOES NOTHING IN THIS VERSION, which took a slice to establish (§6.155).
     *
    ...
    /*
     * RASTCT -- which half of the raster split the interrupt is setting up next.
     *
  ...
  struct Universe
  {
    ...
    // ---- the arena ---------------------------------------------------------------------------
    Stardust dust;         ///< SX, SY, SZ and their fractions, and NOSTM
    PlanetSunState heaps;
    Bubble bubble;
    Ship work{};           ///< The ship the loop is working on
    LineHeap heap;         ///< The `LS%` region, and `SLSP` inside it

    ClipState clip;        ///< The short-range chart's, which the clipper reads
    Projection projection; ///< K3 and K4 -- where the last ship landed on screen
    K3Block axes{};        ///< K3, which `SPS1` fills for the docking check

    // ---- the screen --------------------------------------------------------------------------
    ScreenState screen;   ///< The border, the bank and the mode bytes
    TextState text;
    MessageState message; ///< DLY, de, MCH and messXC

    /// DTW1 to DTW8 -- the sentence machinery's bytes. They were `CharacterPrinter`'s member
    ...
    VideoState video{};         ///< The VIC-II registers `MVTRIBS` reads back

    /*
     * L1M and `l1` -- the 6510's input/output port, which decides what the address space
     * holds (M3-b-3a).
     *
    ...
    std::uint8_t view = 0;      ///< QQ11 -- which screen is up
    std::uint8_t spaceView = 0; ///< VIEW -- which way the player is looking, 0 to 3
    ...
    /*
     * The ship block `LL9` part 1 writes two bytes of directly, as a slot.
     *
    ...
    LaserBurst burst{};
    KeyLogger keys{};
    ControlState control;    ///< JSTX, JSTY and `auto`
    ControlOptions options;
    std::uint8_t explosions = 0;

    /// Non-zero while docked. State the docked half writes and the title screen, the
    ...
    /*
     * QQ9 and QQ10 -- where the crosshairs are on whichever chart is up.
     *
    ...
    /// The seeds of the system under the crosshairs, which `TT111` writes and the
    /// status, system-data and hyperspace screens all read back.
    SystemSeeds selectedSeeds{};

    /// QQ26, QQ19 and AVL -- the market this station is offering, which lasts as long as the
    /// docking does: `GenerateMarket` writes it on arrival and every trading screen reads it.
    MarketState market;

    /*
     * NAME, NA% and DISK -- the commander's FILE, which is not the commander.
     *
    ...
    /// The line editor's buffer, which the original carves out of the ship
    ...
    /*
     * The field width `BPRNT` was last given, and it is state because SV1 does not set it.
     *
    ...
    /*
     * QQ2, QQ28, tek and gov -- the system you are IN, as opposed to the one under the
     * crosshairs.
     *
    ...
    /*
     * The buffer between the game and the raster interrupt (M3-b-2a).
     *
    ...
    /*
     * Music_variables and MUPLA -- the docking music and the title theme (M3-b-2b).
     *
    ...
    /*
     * What `TT17` leaves in X and Y -- the crosshair steps, held between the scan and the
    ...
    /*
     * safehouse and QQ8 -- the system the countdown is running towards, and its distance.
     *
    ...
    /*
     * JSTGY and JSTE -- two of the thirteen that NOTHING ELSE IN THE PORT READS.
     *
    ...
    /// What `MUTOKCH` saw last, which is how it noticed the switch moving. Its
    ...
    /*
     * The sun's heap, which `NWSPS` hands to the SPACE STATION (§6.112).
     *
