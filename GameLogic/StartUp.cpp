...
void CrosshairsToCurrentSystem(const CommanderBlock& _commander, std::uint8_t& _crosshairX,
                               std::uint8_t& _crosshairY) noexcept
{
...
void CurrentSystemToCrosshairs(CommanderBlock& _commander, std::uint8_t _crosshairX,
                               std::uint8_t _crosshairY) noexcept
{
...
ForcedKey ForceKey(std::uint8_t _key, std::uint8_t _dockedFlag, std::uint8_t _view,
                   std::uint8_t _countdown, bool _hyperspaceHeld) noexcept
{
  ForcedKey result{};
...
ForcedKey EnterDockingBay(std::uint8_t& _dockedFlag, std::uint8_t _view, std::uint8_t _countdown,
...
ForcedKey StartGame(GameStart& _game) noexcept
{
  ...
  const std::uint8_t answer =
    _game.effects.ShowTitleScreen(TITLE_LOAD_TOKEN, SHIP_COBRA_MK3, TITLE_COBRA_DISTANCE);
  ...
  if (answer == KEY_YES_INTERNAL)
  {
    ...
    (void)DiskAccessMenu(_game.save, _game.commander, _game.name, _game.image, _game.buffer,
                         _game.useDisk);
  ...
  const NearestSystem found =
    FindNearestSystem(_game.commander.GalaxySeeds(), _game.crosshairX, _game.crosshairY,
