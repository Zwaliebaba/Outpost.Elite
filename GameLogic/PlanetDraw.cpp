...
namespace Elite
{
  ...
  SunRow ClipSunRow(const PlanetSunState& _state, SignMag16 _centre, std::uint8_t _halfWidth, std::uint8_t _row) noexcept
  {
    ...
    {
      /*
       * Even the right-hand end is off the left of the screen. `X1` IS NOT WRITTEN on
    ...
    // A high byte of zero means the right end is already a screen coordinate. Anything else
    ...
    {
      return row; // Both ends are on screen as they stand, and the carry is cleared
    }

    // The left end needs deciding.
  ...
  {
    // FLFLLS and SAL6 -- the heap zeroed from the bottom row upwards.
    //
  ...
  {
    // The heap top back to one and the first x marked empty.
  ...
  void EraseSun(Canvas& _canvas, PlanetSunState& _state) noexcept
  {
    ...
    // `YY(1 0)` is `EDGES`'s centre and a parameter since M2-c-3; what it holds here is where
    // the sun WAS.
    const SignMag16 wasAt{_state.sunX, _state.sunXNext};

    // The literal 143, in the same build where `CHKON` reads `Yx2M1`.
  ...
  void EraseBall(Canvas& _canvas, PlanetSunState& _state) noexcept
  {
    ...
    /*
     * The four endpoint bytes -- and the walk STARTS FROM WHATEVER THE FIRST TWO HOLD. A
    ...
    while (true)
    {
      // Past the heap's top and there is nothing left to erase.
      ...
      {
        /*
         * A break. The pair after it is a new run's START, not another segment's
      ...
      /*
       * `LOIN` swaps its endpoints when it draws right-to-left, and
  ...
  {
    CircleExtent extent;
    // Four sixteen-bit comparisons, and each one's high byte is all that is read.
    ...
    {
      extent.offScreen = true; // Set the carry and return
      return extent;
    }
    ...
    // PL31 -- and the y half also STORES, so the answer is three values rather than a flag.
    ...
    {
      return extent; // PL44 -- which is `PLS6`'s carry clear, not `EDGES`'s (§6.45)
    }
    if (top.value != 0u)
    {
      extent.offScreen = true;
    ...
    // The carry from the last comparison IS the return value.
  ...
  {
    // The segment's far end, as an offset from the circle's centre, with both halves running
    ...
    // The first segment of a circle has a start and no end yet, so it goes straight to
    ...
    {
      // The eight bytes of the two endpoints into the clipper's own workspace.
      Line16 segment;
      ...
      // `XX12(1 0)` -- the far end's y, which `LL145` reads there and `LL9`'s frame owns.
      ...
      if (clipped.rejected)
      {
        endTheRun = true; // Clipped away entirely
      }
      else
      {
        // The clipper may hand the ends back the other way round, and the heap has to
        ...
        /*
         * BL9 / BL8 -- the segment onto the heap.
         *
        ...
        // An end that had to be moved ends the run too, because the next segment does
    ...
    if (endTheRun)
    {
      // A break, unless the last thing written was already one.
    ...
    // This segment's end is the next one's start, and the angle moves on.
  ...
  namespace
  {
    ...
    /*
     * The negate-into-sixteen-bits block, which `CIRCLE2` has twice and `PLS22` twice more.
     *
  ...
  {
    // The flag set and the angle zeroed. `CNT` is the angle this walk is at, `CIRCLE2`'s own
    ...
    {
      /*
       * The two coordinates, a quarter-turn apart in the same table.
       *
      ...
      std::uint8_t high = 0; // The offset's high byte, this loop's own since M2-c-3

      carry = angle >= 33u; // Past the quarter-wave, so the sign has to be put back
      if (carry)
      {
        const Negated negated = NegateWide(across, carry);
        across = negated.low;
        high = negated.high;
        carry = false; // The carry cleared for the addition below
      }

      // PL37 -- and the centre added on, sixteen bits at a time.
      ...
      // The same table a quarter-turn on, which is the cosine.
      ...
      /*
       * Fifteen added to the angle, masked to six bits, and compared against 33.
       *
      ...
      // PL38 -- and the segment is drawn, with the y offset still in X.
      ...
      // Sixty-four steps of one, or eight of eight.
      if (reached >= 65u)
      {
        return; // The carry cleared on the way out
      }
  ...
  {
    // `CIRCLE` wants `CHKON`'s carry only; the extent it also computes is `SUN`'s.
    if (CircleOffScreen(_state, _radius, _centre).offScreen)
    {
      return true;
    }

    // Zeroing entry 0 of the x heap is the flag that tells `WPLS2` there is something to rub out.
    _state.SetBallX(0, 0);

    /*
     * The step halved once above a radius of 8 and again above 60.
     *
  ...
  {
    // The component's two bytes with the sign split off. The index is 9, 11, 21 or
    ...
    /*
     * The quotient's low byte, replaced by 254 when the middle byte is not zero.
     *
  ...
  {
    // PLS1, then * 222/256, and X is SAVED rather than stepped because the caller
    // wants to divide the same axis twice.
    const AxisResult axis = DivideAxisByZ(_ship, _math, _at);

    // The index is saved AFTER the call, and `PLS1` has already stepped it twice -- so what
    ...
    // MULTU scales the axis by 222/256.
    const std::uint8_t scaled = MultiplyUnsigned(axis.value, 222).high;

    // A positive axis returns as it is with a zero high byte. The sign is what `PLS1`
    ...
    // A negative one is negated into a sixteen-bit value whose high half is 255. Negating
  ...
  {
    // The arctangent, then the roof vector's sign decides which way round.
    std::uint8_t angle = Arctan(_numerator, _denominator);

    // The branch SKIPS the flip, so it is the POSITIVE roof vector that gets it.
    ...
    return static_cast<std::uint8_t>(angle >> 2);
  }
  ...
  {
    // Two of PLS1 into the second half of the ellipse's axes.
    AxisResult axis = DivideAxisByZ(_ship, _math, _at);
    const std::uint8_t second = axis.value; // The first of the pair
    _geometry.scaledOrientation[2] = axis.sign;

    axis = DivideAxisByZ(_ship, _math, axis.at);
    _geometry.scaledOrientation[3] = axis.sign;
    return {second, axis.value}; // The ellipse's two scaled axes
  }
  ...
  {
    // The counter zeroed and the flag set. `CNT` is `BLINE`'s segment counter and
    ...
    {
      /*
       * The same quarter-wave table twice, a quarter-turn apart, and each product
       ...
       */
      // The first axis against the sine, both halves; `R` and `K` were the scratch they
      // waited in.
      ...
      // The sign for this quarter, taken as a bit rotated straight out of the comparison
      // against 33 rather than branched on.
      _geometry.scaledOrientation[5] = (coneWidth >= 33u) ? 0x80u : 0x00u;

      // The same table a quarter-turn on -- the cosine -- against the second axis. `K+2` and
      ...
      /*
       * The same sign again for the cosine's quarter -- the angle stepped fifteen, masked
      ...
      // The two `ADD`s, each combining a product with the axis sign it belongs to: (A P) is
      ...
      std::uint8_t offsetHigh = sum.high; // This loop's own since M2-c-3
      std::uint8_t low = sum.low;
      bool carry = sum.carry; // The store and the branch touch no flag, so ADD's carry survives

      // A positive total branches straight past this; a negative one is negated into a
      ...
      // The other pair of products, each half parked in the scratch `ADD` reads.
      ...
      low = sum.low;
      carry = sum.carry; // The flip, the store and the branch again touch no flag
      ...
      // PL43 -- and the segment, with the y offset in X.
      const std::uint8_t reached =
        DrawBallLine(_canvas, _state, _geometry, _math, _clip, _centre, SignMag16{low, offsetHigh}, atAngle, carry, _picture);
      atAngle = reached;

      // The equality test in front of the greater-or-equal one is what makes the last step
  ...
  {
    // A target of 31, then straight into PLS22. Half a turn, because a meridian seen
                        ...
                        const Ship& _ship, Projection& _centre, KBlock _radius, ShipType _type, Picture* _picture) noexcept
  {
    ...
    {
      return; // CHKON refused it
    }

    // A radius whose middle byte is set needed two bytes, which is a planet filling the
    ...
    // A zero in `PLTOG`, the detail switch, skips the markings -- and nothing in this build
    // ever writes it.
    ...
    {
      /*
       * Part 2 -- MERIDIANS. Two great circles at right angles, each drawn as a half
      ...
      // PLS4 -- where the first meridian starts: the nose's z with its sign flipped, over
      // the roof's.
      ...
      _math.k2Low = axis.value; // Into K2, and `MV40` reads this byte a frame later (§8)
    ...
    /*
     * A CRATER. One whole ellipse, offset from the planet's centre along its own
    ...
    // PLS3 from the roof's x -- the offset, one axis at a time, ADDED to x and SUBTRACTED from y.
    ...
    // `P` parks each half of the offset for the one instruction that takes it back out of
    ...
    /*
     * Four PLS1s, each HALVED before it is stored.
     *
    ...
    _math.k2Low = axes.firstX; // Into K2 again -- `MV40`'s byte (§8)
    _geometry.scaledOrientation[0] = axis.sign;
    ...
    // Into PLS22 with a target of 64 from an angle of zero -- a whole turn.
  ...
  {
    /*
     * Three rejections before any arithmetic.
     *
    ...
    // The radius is 96 * 256 / z, and 96 is the planet's size in the same units everything
    ...
    // A radius that overflowed a byte is clamped to 248, and the middle byte is LEFT AS IT
    ...
    // An ODD type is the sun and an even one the planet -- bit 0 shifted out into the carry
  ...
  {
    // Entry 0 stops being the "nothing there" flag the moment the routine commits to
    ...
    /*
     * `CNT`, the roughness mask, built out of the radius rather than looked up.
     *
     ...
     */
    std::uint8_t roughness = 0; // `SUN`'s own since M2-c-3
    ...
    /*
     * PLF18 -- where to stop. `CHKON` left the circle's top and bottom in `P+1` and `P+2`,
     ...
     */
    std::uint8_t stopAt = _state.lowestVisibleRow; // `SUN`'s own too
    ...
    /*
     * How far the bottom visible row is from the sun's centre, as a sixteen-bit subtraction.
    ...
    {
      // The centre is BELOW the bottom row, so the walk starts above it and the
    ...
    {
      // Further than a byte, so start at the sun's own edge.
      at = _radius;
      sign = 0;
    }
    else if (at == 0u)
    {
      sign = 0xFF; // Exactly on the centre
    }
    else if (at >= _radius)
    {
      at = _radius; // The branch to PLF5 not taken, so PLF4
      sign = 0;
    }

    // The distance from the centre and its sign, stored where `PLF6` reads them.
    _state.v = at;
    _state.vNext = sign;

    // The radius squared, which every row's half-width is a square root of. `SUN`'s own
    ...
    // Part 2 -- rub out the rows BELOW the sun, with last frame's centre, before any of
    // this frame's arithmetic touches `YY`.
    std::uint8_t row = _state.lowestVisibleRow;
    const SignMag16 wasAt{_state.sunX, _state.sunXNext}; // YY(1 0) -- where the sun was
    const SignMag16 isAt{_centre.x, _centre.x1};         // YY(1 0) again -- and where it is
    ...
    /*
     * The body, one screen row at a time.
     *
     ...
     */
    /*
     * `PLF6`'s row-zero exit leaves through the ROUTINE'S TAIL and not through part 4, so a
    ...
    for (;;)
    {
      // The half-width, as sqrt(K^2 - v^2). `T` held the square's high byte and (R Q) the
      ...
      _math.lastDivisor = root.value; // What LL5 leaves in Q -- and the last row's root is the frame's Q when the sun is the last slot drawn

      // The ragged edge -- a random byte masked by `CNT` and added to the half-width -- and
      ...
      // The old width is read out of the heap and the new one stored in its place,
      ...
      {
        // Nothing was there last frame, so the whole of the new line is drawn.
        const SunRow fresh = ClipSunRow(_state, isAt, _state.sun[row], row);
        if (fresh.offScreen)
        {
          _state.sun[row] = 0;
      ...
      // The row steps up, and row zero ends the walk through the routine's tail.
      --row;
      if (row == 0u)
      {
        break;
      ...
      if (_state.vNext != 0u)
      {
        // Above the centre, so the distance GROWS, and once it passes the radius
      ...
      {
        /*
         * Below the centre, so the distance SHRINKS as the walk climbs.
         *
    ...
    // Part 4 -- rub out whatever is left above the sun, again with last frame's centre.
    if (eraseAbove)
    {
      while (row != 0u)
      {
  ...
  {
    // Every byte of the block zeroed, top down.
    _work = Ship{};

    /*
     * And then three bytes go back in, 96 in each and the third with its top bit set.
     *
  ...
  {
    /*
     * Three random bytes per speck, and the generator is threaded straight through.
     *
    ...
    for (std::uint8_t at = _dust.count; at != 0u; --at)
    {
      ...
      const std::uint8_t distance = static_cast<std::uint8_t>(roll.value | 8u);
      _dust.z[at] = distance;

      roll = _rng.Next(roll.carry);
      _dust.x[at] = roll.value; // SX,Y and X1, both
      const std::uint8_t x1 = roll.value;

      roll = _rng.Next(roll.carry);
      _dust.y[at] = roll.value; // SY,Y and Y1, both
  ...
  {
    // The slot list walked from the start, with the two cases the body handles.
    for (std::size_t slot = 0; slot < _bubble.slots.size(); ++slot)
    {
      const ShipType type = TypeOf(_bubble.slots[slot]);
      if (type == ShipType::None)
      {
        break; // The first empty slot ends the list
      }
      if (IsBody(type))
      {
        continue; // The planet and the sun have no blip and no line heap
      }

      // Thirty-two bytes copied out of the slot, not the whole block: the AI byte, the heap
      ...
      // Both stores are the routine's own and were invisible while the scanner was a seam:
      ...
      /*
       * The state byte read back through the slot pointer, masked, and put where it came from.
       *
    ...
    // The heap pointer to zero and 255 into entry 0 of both halves. Note `LSP` goes
    // to ZERO here and to one in `WP1`; the two are not the same reset.
    _state.ballHeapTop = 0;
    _state.SetBallX(0, 0xFF);
    _state.SetBallY(0, 0xFF);

    // And the fall-through into FLFLLS.
    ClearSunHeap(_state);
  }
  ...
  {
    // `QQ11` is the view, zero for the space view, and a menu has no stardust to
                          ...
                          Picture* _frame) noexcept
  {
    ...
    for (;;)
    {
      ...
      // 160 is half the screen's width, so a ring is abandoned once it is wider than the
                           ...
                           Presenter& _present, Picture* _picture, Picture* _frame) noexcept
  {
    ...
    // Eight rings, and `XX4` is this loop's counter. Nothing reads it afterwards:
