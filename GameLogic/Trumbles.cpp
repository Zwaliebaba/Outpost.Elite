...
namespace Elite
{
  ...
  {
    /*
     * The main loop counter masked to three bits and compared against `TRIBCT`.
     *
    ...
    // The turn DOUBLED into Y, which indexes the three RAM tables at two bytes a Trumble,
    ...
    // The video chip's registers, mapped in.
    SetMemoryMap(_map, MEMORY_MAP_IO);

    /*
     * A random byte compared against 235.
     *
    ...
    {
      // Two bits of the roll index the direction tables, high byte and low.
      ...
      /*
       * A second roll, two bits again, into the y velocity.
       *
    ...
    /*
     * MVTR1 masks this sprite's ninth x bit out of the shared register -- and there is no
     * line for it here.
     *
    ...
    /*
     * The sprite's y plus its y velocity, straight back into the register.
     *
    ...
    // The low byte of x plus its velocity -- and it READS the register back, which is why
    ...
    /*
     * nominus and oktrib -- the ninth bit masked, then the right edge tested.
     *
    ...
    // The shadow the game keeps because the shared register cannot be read back
    // one sprite at a time.
    _sprites.coordinateXHigh[at] = high;

    /*
     * The ninth bit ORed back into the shared register when it is set, then the
    ...
    // SETL1 again and NOMVETR -- the registers mapped back out, and the jump back into the
