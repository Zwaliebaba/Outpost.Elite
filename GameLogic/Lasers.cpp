 ...
 */
bool DrawLaserPair(Canvas& _canvas, DrawWorkspace& _draw, const LaserBurst& _burst,
                   std::uint8_t _left, std::uint8_t _right) noexcept
{
...
bool DrawLaserLines(Canvas& _canvas, DrawWorkspace& _draw, const LaserBurst& _burst,
               ...
               bool _carryIn) noexcept
{
  ...
  const AddResult y =
    AddWithCarry(static_cast<std::uint8_t>(down.value & 7u),
                 static_cast<std::uint8_t>(VIEW_CENTRE_Y - 4u), down.carry);
  _burst.y = y.value;

  // 6502: JSR DORND / AND #7 / ADC #X-4 / STA LASX -- the same again, across.
  const RngResult across = _rng.Next(y.carry);
  const AddResult x =
    AddWithCarry(static_cast<std::uint8_t>(across.value & 7u),
                 static_cast<std::uint8_t>(VIEW_CENTRE_X - 4u), across.carry);
  _burst.x = x.value;
