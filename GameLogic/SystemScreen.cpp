...
void PrintInhabitants(TokenPrinter& _printer, const SystemSeeds& _seeds) noexcept
{
  ...
  const std::uint8_t noun =
    static_cast<std::uint8_t>(((_seeds.bytes[5] & 0x03u) + appearance) & 0x07u);
                      ...
                      std::uint16_t _distance) noexcept
{
  ...
  PrintTitleLine(_screen.printer, _screen.text,
                 static_cast<std::uint8_t>(ECONOMY_KIND + (_data.economy >> 2)));

  // 6502: LDA #162 / JSR TT68 / LDA QQ4 / CLC / ADC #177 / JSR TT60.
  PrintThenColon(_screen.printer, GOVERNMENT_HEADING);
  PrintTitleLine(_screen.printer, _screen.text,
                 static_cast<std::uint8_t>(GOVERNMENT_FIRST + _data.government));
  ...
  const std::uint16_t radius = static_cast<std::uint16_t>(
    (static_cast<std::uint16_t>((_seeds.bytes[5] & 0x0Fu) + RADIUS_HIGH_BASE) << 8) | _seeds.bytes[3]);
