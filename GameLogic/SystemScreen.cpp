...
namespace Elite
{
  ...
  {
    /*
     * The tokens TT25 prints, in the order it prints them.
     *
    ...
    /// Column 9, where the title starts.
    constexpr std::uint8_t TITLE_COLUMN = 9;

    /*
     * The three `CMP` bounds in the species block, which are not the same number.
     *
    ...
    /// Four bits of a seed byte plus 11 -- the radius's high byte, which is why no planet in
    /// Elite is smaller than 2,816 km or larger than 6,911.
    constexpr std::uint8_t RADIUS_HIGH_BASE = 11;

    /*
     * TT75 through TT207 -- the inhabitants.
     *
    ...
    {
      // The seed byte shifted twice and PUSHED, because the next two words read different
      ...
      // Three bits, and anything past the table branches to `TT205` and skips the word.
      ...
      // Pulled back and shifted three more, so five shifts in all from `QQ15+5`.
      ...
      /*
       * Two seed bytes folded and masked to three bits, into `QQ19`.
       *
      ...
      // Two bits of a seed byte added to `QQ19`, masked, and used as a token.
  ...
  {
    // The two distance bytes tested together -- no distance at all skips to `INCYC`, and
    // no newline either.
    if (_distance == 0)
    {
    ...
    // Token 191 through `TT68`.
    PrintThenColon(_printer, DISTANCE_HEADING);

    // `pr5` with five digits and ONE decimal place, so `QQ8` is in tenths of a light year
    // and 65535 would read as 6553.5.
    PrintValue(_characters, _distance, 5, true);

    // Token 195, and then the routine FALLS INTO `TT60` -- so the light years, the cursor
  ...
  {
    // TRADEMODE on view 1, which sets the cursor and the case flags too.
    ...
    // Column 9, then token 163 through `NLIN3` -- the rule `NLIN3` falls into is the
    ...
    // A row down, sentence case, and a newline.
    ...
    // Token 194 through `TT68`.
    PrintThenColon(_ports.printer, ECONOMY_HEADING);

    /*
     * TT70 and TT71 -- the economy stepped up and halved, then compared against 2.
     *
    ...
    if (prosperity == 2u)
    {
      // Token 173, then straight to `TT72`, which lands PAST the addition above.
    ...
    else
    {
      ...
      if (prosperity > 2u)
      {
        // Reached with the carry SET by the comparison, so it is a plain subtraction of
    ...
    // The economy halved twice, plus 168, through `TT60`.
    PrintTitleLine(_ports.printer, _universe.text, static_cast<std::uint8_t>(ECONOMY_KIND + (_data.economy >> 2)));

    // Token 162 through `TT68`, then the government plus 177 through `TT60`.
    ...
    /*
     * Token 196 through `TT68`, then the tech level stepped up and printed by `pr2`.
     *
    ...
    // Token 192 through `TT68`, then `pr2` with a decimal point -- three digits, so the
    ...
    // An open bracket, then the seed byte's sign decides which species word.
    ...
    {
      // Token 188, then straight to `TT76`.
      _ports.printer.Print(HUMAN_COLONIALS);
    }

    // An "S" and a close bracket. The plural is unconditional, which is why the
    // screen says "Human Colonials" and never "Human Colonial".
    _ports.printer.Print('S');
    PrintTitleLine(_ports.printer, _universe.text, ')');

    // Token 193 through `TT68`, then `pr6` on the productivity pair and a space.
    ...
    /*
     * `QQ17` cleared, an "M", then token 226 through `TT60`.
     *
    ...
    // Token 250 through `TT68`.
    PrintThenColon(_ports.printer, RADIUS_HEADING);

    // Four bits of one seed byte plus 11 as the high byte, another as the low, through `pr5`.
    ...
    // Both letters through the CHARACTER printer, so the case flags do not touch them and
    // the "km" stays lower case whatever `QQ17` holds.
    _ports.characters.Put('k');
    _ports.characters.Put('m');

    // TTX69, then PDESC -- and PDESC's mission overrides are phase 4's, which is what
