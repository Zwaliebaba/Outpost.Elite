...
namespace Elite
{
  ...
  {
    /*
     * The whole handler is indexed by RASTCT, and the index is 0 or 1 and nothing else.
     *
    ...
    // `zebop` indexed by the counter -- which block of screen RAM colours this half.
    out.memoryPointers = spaceView ? RASTER_MEMORY_SPACE_VIEW : _screen.colourBank;

    // `moonflower` into the second control register -- bit 4 is the multicolour bit, so
    // this is the register the energy bomb changes.
    out.control2 = spaceView ? _screen.upperBitmapMode : _screen.bitmapMode;

    // `shango` into the raster compare -- where the NEXT interrupt fires, which is what
    // alternates.
    out.nextRasterLine = RASTER_NEXT_LINE_TABLE[index];

    /*
     * `santana` into the sprite multicolour register and `lotus` into sprite 1's colour.
     *
    ...
    /*
     * The energy bomb's top bit decides whether `welcome` is incremented.
     *
    ...
    // `welcome` indexed by the counter into the background register -- the space view's
    ...
    // `innersec` indexed by the counter, written back as the next counter.
