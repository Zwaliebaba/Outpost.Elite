...
DiskMenuResult DiskAccessMenu(SaveScreen& _screen, CommanderBlock& _block,
                              ...
                              std::span<std::uint8_t> _buffer, bool& _useDisk) noexcept
{
  ...
  for (;;)
  {
    ...
    if (key == DISK_MENU_LOAD)
    {
      ...
      (void)AskCommanderName(_screen.keys, _screen.chpr, _screen.text, _screen.extended, _screen.effects,
                             _buffer, imageName, limits);
