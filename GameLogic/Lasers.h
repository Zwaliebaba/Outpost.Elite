#pragma once

#include <cstdint>

#include "Canvas.h"
#include "Dashboard.h"
#include "Picture.h"
#include "Rng.h"

namespace Elite
{

  /*
   * The player's laser (slice 3d-c).
   *
   * Four lines from the corners of the view to a point that jitters near the centre, drawn with
   * EOR so the next frame takes them off again. The jitter is what makes the beam look alive, and
   * it is two random bytes -- which is why this routine is where the laser's heat comes from too:
   * the same instruction stream that picks the endpoint adds eight to the laser temperature.
   */

  /// X and Y -- the centre of the 256 x 144 space view, which every laser line converges on.
  inline constexpr std::uint8_t VIEW_CENTRE_X = 128;
  inline constexpr std::uint8_t VIEW_CENTRE_Y = 72;

  /// What one shot costs in laser heat, added to `GNTMP`.
  inline constexpr std::uint8_t LASER_HEAT_PER_SHOT = 8;

  /// LASX and LASY -- where the beams converge this frame, which persists so the next frame
  /// can rub the same lines out.
  struct LaserBurst
  {
    std::uint8_t x = 0;
    std::uint8_t y = 0;
  };

  /// What `LASLI` reaches outside this slice.
  /*
   * Fire: pick the convergence point, heat the laser, drain the banks, draw.
   *
   * THREE UNCLEARED ADDS IN NINE INSTRUCTIONS, and §6.65's split runs right through them. The two
   * coordinates read `DORND`'s exit carry, which the generator decides -- so the convergence point
   * spans NINE rows and nine columns where three random bits alone would give eight, and a sweep
   * counting distinct values is what proves it. The third add, the heat one, reads the carry the
   * second coordinate left, and that one cannot be set: three bits plus 124 plus at most one is
   * 132. So a shot costs exactly eight heat, and the port asserts that rather than assuming it
   * (§6.68).
   *
   * Returns the exit carry, because `LL30` leaves one and the flight loop's next instruction is an
   * `ADC`. The port returns it rather than guessing.
   */
  [[nodiscard]] bool FireLaser(Canvas& _canvas, Rng& _rng, LaserBurst& _burst, FlightStatus& _status,
                               std::uint8_t _view, bool _carryIn, Picture* _picture = nullptr) noexcept;

  /*
   * Draw the four lines without firing, which is how the beam is rubbed out.
   *
   * A chart on screen sends it to `LASLI-1`, the byte before the routine, which is the previous
   * one's return borrowed as a branch target. So a chart on screen means no laser at all.
   */
  [[nodiscard]] bool DrawLaserLines(Canvas& _canvas, const LaserBurst& _burst, std::uint8_t _view,
                                    Picture* _picture = nullptr) noexcept;

} // namespace Elite
