...
void ChangeView(FlightScreen& _screen, std::uint8_t _to) noexcept
{
  ...
  if (_screen.view != 0u)
  {
    ...
    DrawLaserSights(_screen.canvas, _screen.math, _screen.commander, _screen.trumbleSprites,
                    _screen.spaceView, _screen.sight); // 6502: JSR SIGHT

    // 6502: JMP NWSTARS -- a whole new field, because there was no space view to keep.
    SeedStardustAndClearShips(_screen.canvas, _screen.draw, _screen.dust, _screen.rng,
                              _screen.heaps, _screen.bubble, _screen.work, _screen.flight,
                              _screen.view, false);
    return;
  }
  ...
  ClearAllShips(_screen.canvas, _screen.draw, _screen.heaps, _screen.bubble, _screen.work,
                _screen.flight, _screen.view);

  DrawLaserSights(_screen.canvas, _screen.math, _screen.commander, _screen.trumbleSprites,
                  _screen.spaceView, _screen.sight);
}
