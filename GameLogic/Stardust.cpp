...
WideResult MultiplyByHeight(MathWorkspace& _math, DrawWorkspace& _draw, const Stardust& _dust,
                        ...
                        const FlightState& _flight, Stardust& _dust, Rng& _rng) noexcept
{
  for (std::uint8_t at = _dust.count; at != 0u; --at)
  {
    ...
    if (killed)
    {
      ...
      const AddResult renewed =
        AddWithCarry(static_cast<std::uint8_t>(roll.value & 0x7Fu), 10u, roll.carry);
