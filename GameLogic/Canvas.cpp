 ...
 */
void ResolveCell(std::uint8_t* _out, int _outStride, const std::uint8_t* _bitmap, std::uint8_t _cellByte,
                 std::uint8_t _colourRam, std::uint8_t _background, bool _multicolour) noexcept
{
  ...
  for (int cellRow = 0; cellRow < CELL_ROWS; ++cellRow)
  {
    ...
    for (int cellColumn = 0; cellColumn < CELL_COLUMNS; ++cellColumn)
    {
      ...
      ResolveCell(_out.data() + static_cast<std::size_t>(cellRow) * 8 * WIDTH + cellColumn * 8, WIDTH,
                  &m_screen[cellRow * ROW_BYTES + cellColumn * 8], m_screen[cellBase + cell], m_colourCells[cell],
                  m_background, lower);
    }
  }
}
