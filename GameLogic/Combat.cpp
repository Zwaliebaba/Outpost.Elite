 ...
 */
[[nodiscard]] std::uint8_t Volume(std::uint8_t _distance, std::uint8_t _first, std::uint8_t _second,
                                  std::uint8_t _third, std::uint8_t _fourth) noexcept
{
                        ...
                        std::uint8_t _type) noexcept
{
  ...
  const AddResult fraction =
    AddWithCarry(commander.At(Field::KillsLow), ShipByte(table), false);
  commander.At(Field::KillsLow) = fraction.value;

  const AddResult whole =
    AddWithCarry(commander.At(Field::Kills),
                 ShipByte(static_cast<std::uint16_t>(table + SHIP_TYPE_COUNT)), fraction.carry);
  ...
  if (whole.carry)
  {
    ...
    ShowMessage(_screen.canvas, _screen.printer, _screen.text, _screen.extended, _screen.message,
                MESSAGE_RIGHT_ON_COMMANDER, _screen.view);
  }
...
bool TakeDamage(FlightScreen& _screen, DashboardEffects& _effects, const ShipBlock& _target,
                std::uint8_t _damage, bool _carryIn) noexcept
{
