...
namespace Elite
{
  ...
  {
    _video.colour[0] = _colour; // 6502: sprite 0's colour register
  }

  void ApplySpritesEnabled(VideoState& _video, std::uint8_t _mask) noexcept
  {
    _video.enabled = _mask; // 6502: the sprite enable register
  }

  void ApplyMaskSprites(VideoState& _video, std::uint8_t _mask) noexcept
  {
    // 6502: the sprite enable register read, masked to its low two bits and written back -- and
  ...
  {
    _video.expanded = _mask; // 6502: both sprite expand registers, vertical and horizontal
  }
  ...
  {
    // 6502: NOSPRITES -- the enable byte zeroed, and nothing else. The positions and colours are
