...
namespace Elite
{
   ...
   *
   * K% and LS% -- the blocks grow UP from `K%` and the heap grows DOWN from `LS%`, and `NWSHP`
   ...
   */
  inline constexpr std::uint16_t SHIP_BLOCK_BASE = 0xF900;
  inline constexpr std::uint16_t SHIP_HEAP_TOP = 0xFFC0;   ///< LS%, where SLSP starts

  /// The SUN's line heap, which `NWSPS` hands to the space station: nowhere near the
  /// arena, and the reason `LineHeap` takes a window on loan (§6.112).
  inline constexpr std::uint16_t SUN_HEAP_ADDRESS = 1408;

  /*
   * XX19(1 0) and SLSP -- a place in the ship line heap, as bytes UP from `K%` (ruling R-c: the
  ...
  struct HeapOffset
  {
    ...
    /// (XX19),Y -- the Y-th byte of the run that starts here.
    [[nodiscard]] constexpr HeapOffset Byte(std::uint16_t _index) const noexcept
    {
      return {static_cast<std::uint16_t>(up + _index)};
    }
    /// SLSP moving down, or `KILLSHP`'s `P` -- a run that starts this many bytes lower.
