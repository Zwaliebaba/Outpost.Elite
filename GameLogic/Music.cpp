...
namespace Elite
{
  ...
  namespace
  {
    /// BIT / BMI -- every music flag is read from its top bit.
    [[nodiscard]] bool IsSet(std::uint8_t _flag) noexcept
    {
      return (_flag & 0x80u) != 0u;
    }

    /// The volume register's low nibble -- full volume, no filter.
    constexpr std::uint8_t FULL_VOLUME = 0x0Fu;

    /// The vibrato periods, in interrupts, as the GMA release has them.
    constexpr std::uint8_t VIBRATO_PERIOD_VOICE_3 = 5;
    constexpr std::uint8_t VIBRATO_PERIOD_VOICE_2 = 4;

    /// How far above the note each voice's vibrato frequency sits.
    ...
    /// SID+&7, SID+&8 and SID+&E, SID+&F -- voice 2 and 3's frequency registers.
    ...
    /// SID+&4, SID+&B and SID+&12 -- the three control registers.
    ...
    /*
     * The data pointer stepped, carrying into its high byte, and then read.
     *
    ...
    /// Two bytes into voice 1's frequency, high register first.
    ...
    /*
     * Voice 2's frequency, and the two copies the vibrato alternates between.
     *
    ...
    /// The same for voice 3, and 37 rather than 32 in this release.
    ...
    /// BDlab4, BDlab6, BDlab8 -- a zero into the control register and then the value, so the
    ...
    /*
     * BDlab1, BDlab23, BDlab24 -- the vibrato, and then BDlab21.
     *
    ...
    {
      // Voice 3's counter stepped and compared against its period.
      ...
      // Voice 2's counter stepped and compared against its period.
    ...
    /*
     * Startat2 and what follows it -- the checks every start goes through.
     *
    ...
    void StartAt(MusicPlayer& _music, std::uint16_t _tuneStart, MemoryMap& _map, SidWriteLog& _log) noexcept
    {
      ...
      StartDockingMusicNow(_music, _map, _log); // The fall-through into `april16`
    }
  } // namespace

  void StartDockingMusicNow(MusicPlayer& _music, MemoryMap& _map, SidWriteLog& _log) noexcept
  {
    // The I/O page in, so the SID exists to be written.
    SetMemoryMap(_map, MEMORY_MAP_IO);

    BeginTune(_music, _log);
    _music.playing = 0xFFu;  // MUPLA set

    // The map back to RAM, shared with `stopat`'s exit.
    SetMemoryMap(_map, MEMORY_MAP_RAM);
  }

  void StartDockingMusic(MusicPlayer& _music, MemoryMap& _map, SidWriteLog& _log) noexcept
  {
    // The "docking plays the theme" option picks which of the two tunes starts.
  ...
  {
    // The theme's address, one byte back because the fetch pre-increments.
  ...
  {
    // A RESET reached from inside `TITLE` stops nothing, so the title music
    // survives it.
    ...
    // Forced music is not stopped, it is started.
  ...
  {
    // Nothing to stop unless it is playing.
    if (!IsSet(_music.playing))
    {
      return;
    }

    // The effects flushed, the I/O page in, and `MUPLA` cleared.
    FlushSoundEffects(_buffer);
    SetMemoryMap(_map, MEMORY_MAP_IO);
    _music.playing = 0u;

    // Twenty-five zeros with interrupts off, from the top register down to the
    // first.
    ...
    // The volume back to full, and interrupts on again.
    _log.Add(SID_VOLUME, FULL_VOLUME);

    // The map back to RAM, and `april16` shares this exit.
    SetMemoryMap(_map, MEMORY_MAP_RAM);
  }

  void BeginTune(MusicPlayer& _music, SidWriteLog& _log) noexcept
  {
    // The buffer, the rest counter and both vibrato counters zeroed.
    _music.buffer = 0u;
    _music.counter = 0u;
    _music.vibrato2Count = 0u;
    _music.vibrato3Count = 0u;

    // The registers zeroed from the top down, and the loop STOPS AT 1, so the
    ...
    // The start address into both pointer pairs -- the one that reads and the one that
    // restarts.
    _music.pointer = _music.tuneStart;
    _music.restart = _music.tuneStart;

    // The volume to full on the way out.
    _log.Add(SID_VOLUME, FULL_VOLUME);
  }

  void RunMusic(MusicPlayer& _music, SidWriteLog& _log) noexcept
  {
    // A non-zero rest counter comes down by one and the pass ends there.
    if (_music.counter != 0u)
    {
    ...
    {
      /*
       * BDskip1, BDLABEL and BDLABEL2 -- where the next command comes from.
       *
      ...
      // BDJMPTBL, BDJMPTBH -- the jump table, which is the switch below. Indexing it with
      ...
      case 7: // The three attack/decay registers, then the three sustain/release.
      ...
      case 15:
        /*
         * A 1 rotated in at the bottom and shifted up three, so the buffer gains an
      ...
      case 8: // The rest length into the counter, then the vibrato and out.
        ...
        break; // A rest of zero falls straight back into BDskip1

      case 9:
      case 11: // BDRO11 -- which is a jump to BDRO9
        _music.buffer = 0u;
        _music.pointer = _music.restart;
        break;

      case 10: // The three pulse widths, low register then high.
      ...
      case 14: // Volume and filter mode, filter control, filter cut-off high.
  ...
  {
    // The music runs first when it is playing, and the "effects during music" option decides
