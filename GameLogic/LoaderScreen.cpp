...
namespace Elite
{
  ...
  namespace
  {
    /// &2D0 -- the offset of the dashboard within a block of screen RAM or colour RAM, which
    ...
    /// The two cells the border box is drawn down.
    constexpr int BORDER_LEFT_CELL = 3;
    constexpr int BORDER_RIGHT_CELL = 36;

    /*
     * LOOP10 and LOOP11 -- the border box's palette down one block of screen RAM.
     *
    ...
    void ColourBorderBox(Canvas& _canvas, std::uint16_t _block, int _rows, Picture* _picture) noexcept
    {
      for (int row = 0; row < _rows; ++row)
      {
        ...
        // The right edge FIRST, because the left one is reached by decrementing the index
        ...
        // Cells 37, 38 and 39, written out three times rather than looped -- the original's
  ...
  {
    /*
     * Part 5's first loop, which runs the pages from &40 up to &60.
     *
    ...
    /*
     * .LOOP3 / .LOOP4 with A = &10 and X counting to &68 -- BOTH blocks of screen RAM,
    ...
    // ZP = SCBASE+&2400+&2D0, (A ZP2) = sdump, JSR mvsm -- 280 bytes, the dashboard's own
    ...
    // The border box down the text view's block, all 25 rows.
    ColourBorderBox(_canvas, Canvas::SCREEN_CELLS, Canvas::CELL_ROWS, _picture);

    // LOOP11 -- and down the space view's block, which stops at the dashboard.
    ColourBorderBox(_canvas, Canvas::DASHBOARD_CELLS, Canvas::DASHBOARD_CELL_ROW, _picture);

    /*
     * Thirty-two cells of the bottom row, counting down.
     *
    ...
    /*
     * Part 6's .LOOP19 -- four pages of colour RAM, &D800 to &DBFF, zeroed.
     *
    ...
    // ZP = COLMEM+&2D0, (A ZP2) = cdump, JSR mvsm -- the same 280 cells again, in the other
    ...
    /*
     * Thirty-four cells from the top row's third, counting down.
     *
    ...
    /*
     * Part 4's store to the background register, which is multicolour %00.
     *
  ...
  {
    _video.enabled = 0u;     // All eight sprites off
    _video.expanded = 0xFFu; // All eight double height and double width

    // The six Trumbles' colours, all nine.
    ...
    // Bit 9 of every x cleared, which the sixteen-bit x carries.
    // The sights, in the centre of the view.
    _video.x[0] = 161u;
    _video.y[0] = 101u;

    // The first Trumble at 18 and each one after it DOUBLED -- 18, 36, 72, 144 -- then a
