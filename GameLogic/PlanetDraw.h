 ...
 * rewound so the next `CIRCLE2` overwrites the same run, and each circle is EORed over the last.
 * The radius starts at `(_index AND 7) + 8` and doubles, so the eight rings are eight different
 * starting sizes crossing the screen at eight different times.
 *
