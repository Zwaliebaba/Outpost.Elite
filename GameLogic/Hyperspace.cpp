...
namespace Elite
{
  ...
  namespace
  {
    ...
    /// 6502: the view `MJP` and a completed jump both clear to.
    constexpr std::uint8_t SPACE_VIEW = 3;

    /// 6502: one constant doing two jobs -- the Thargoid count witchspace stops at, and the
    ...
    /// 6502: MJP -- three bytes in 256 miss the system entirely.
    constexpr std::uint8_t WITCHSPACE_ROLL = 253;

    /// 6502: the "GALACTIC HYPERSPACE" token, through `MESS`.
    constexpr std::uint8_t GALACTIC_MESSAGE = 116;

    /// 6502: the middle of the galaxy, where `Ghy` puts you: both crosshair bytes.
    constexpr std::uint8_t GALAXY_CENTRE = 96;

    /// 6502: into MJ -- `ZINF`'s loop counter, which reaches &FF and is never reloaded.
    constexpr std::uint8_t WITCHSPACE_FLAG = 0xFFu;

    /*
     * 6502: MJP1 -- a Thargoid made, then the count tested; and the test is BACKWARDS from the
  ...
  {
    /*
     * 6502: TT111 -- and `TT18` enters at `hyp1+3`, three bytes past it, because the chart has
    ...
    if (_findNearest)
    {
      ...
      // 6502: `TT111` ends by jumping to `TT24`, so `QQ3` to `QQ5` are its side effect and not its answer.
      _described = nearest.data;

      /*
       * 6502: `TT111` SNAPS THE CROSSHAIRS onto the system it found, both bytes.
       *
    ...
    // 6502: jmp -- where you are becomes where the crosshairs are, which is what arriving
    // means.
    CurrentSystemToCrosshairs(_universe.commander, crosshairX, crosshairY);

    // 6502: TT112 -- the six seeds the countdown saved, because the player has been moving the
    // crosshairs ever since.
    _universe.current.seeds = _target;

    // 6502: X came out of the loop at &FF and stepping it makes zero, so arriving resets the
    // spawner's rate limit (§6.135). One instruction doing two things.
    _universe.explosions = 0;

    /*
     * 6502: the economy, tech level and government copied out of the seed bytes.
     *
  ...
  void EnterWitchspace(Universe& _universe, Ports& _ports, Commander& _commander) noexcept
  {
    ...
    // 6502: the space view, the tunnel, then `RES2`.
    SetUpScreen(_universe, _ports, SPACE_VIEW);
    DrawHyperspaceTunnel(_universe, _ports);
    ResetShipAndBubble(_universe, _ports);

    /*
     * 6502: into MJ -- and Y is whatever `RES2` left, which is not a value this routine chose.
     *
    ...
    // 6502: LOOK1 on view 0 -- the forward view, drawn over what the tunnel left.
    ChangeView(_universe, _ports, 0u);

    /*
     * 6502: the y coordinate's low five bits complemented, so leaving witchspace does not put
  ...
  {
    /*
     * 6502: ptg -- shift out, set the carry, rotate back: which sets bit 0 and is NOT a rotate
     * of the byte.
     *
                         ...
                         std::uint8_t _crosshairY, const SystemSeeds& _galaxy, bool _controlHeld, bool _authorNames) noexcept
  {
    ...
    /*
     * 6502: the jump's cost taken off the tank, and a borrow clamped to empty.
     *
    ...
    // 6502: ee5 -- the tunnel is only drawn from a space view. Jumping with a chart up spends
    ...
    // 6502: ee5 -- `CTRL` masked against `PATG`: the configuration key and the option together.
    ...
    // 6502: MJP -- a random byte of 253 or more, and three bytes in 256 miss.
    ...
    // 6502: hyp1+3 -- past the call to `TT111`, because the chart has already chosen.
    ArriveAtSystem(_universe, _selected, _jump.target, _described, _market, _crosshairX, _crosshairY, _galaxy, false);

    // 6502: RES2 then SOLAR -- a clean bubble and then the system's own planet and sun.
    ResetShipAndBubble(_universe, _ports);

    // 6502: SOLAR -- the four seams it reached through are four routines this library now
    // contains, so M3-b-1 has it call them.
    BuildSystem(_universe, _ports, false);

    /*
     * 6502: RTS111 -- the view masked to six bits, and anything left goes straight back.
     *
    ...
    // 6502: TTX66, then TT114 on a non-zero view; otherwise the view steps up and it falls into `TT110`.
    ...
    {
      // 6502: TT114 -- and that is a jump OUT of this routine into the chart's own redraw,
      // not a return. The caller does it, the way it does the launch below.
      return JumpResult::RedrawChart;
    }

    ++_universe.view; // 6502: the view stepped up, and the fall-through into `TT110` is the caller's
                    ...
                    ChartView& _chart) noexcept
  {
    ...
    /*
     * 6502: no drive fitted branches to `zZ+1` -- and there is no code at `zZ+1`.
     *
    ...
    // 6502: X was 255 and stepping it makes zero, so both bytes become zero: the drive is spent
    // and the record is clean, from one register.
    _universe.commander.galacticDrive = 0u;
    _universe.commander.legalStatus = 0u;

    // 6502: wW2 with a two -- the countdown started at two rather than fifteen, and `wW2` stores
    // the same value into `QQ22` as well as `QQ22+1` (§6.159).
    _jump.countdown = 2u;
    _jump.counter = 2u;

    // 6502: the galaxy number stepped up and masked -- eight galaxies, and the mask is what
    ...
    /*
     * 6502: G1 -- each of the six seed bytes rotated left by one.
     *
    ...
    // 6502: zZ -- the crosshairs to the middle of the new galaxy, both bytes.
    _chart.cursorX = GALAXY_CENTRE;
    _chart.cursorY = GALAXY_CENTRE;

    // 6502: TT110 -- and this is the LAUNCH, called for its redraw: a galactic jump from a
    // chart leaves you in space looking forward.
    Launch(_universe, _ports, _chart.cursorX, _chart.cursorY, _selected);

    // 6502: TT111, then the six seeds copied into `safehouse` -- the system nearest the middle
    ...
    // 6502: both distance bytes zeroed, so the distance is zero and the countdown has nowhere
    // to go.
    _jump.distance = 0u;

    /*
     * 6502: the message token, and then it FALLS INTO `jmp` and returns.
     *
