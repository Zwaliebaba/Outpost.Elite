...
namespace Elite
{
  ...
  {
    // No dashboard on any view but the space view, so no scanner.
    if (_view != 0u)
    {
      return;
    }

    // Bit 4 of the state byte is "show this on the scanner", and it is cleared for the ships
    ...
    // A negative type goes to SCR1 -- the planet and the sun are 128 and 129, not shown.
    if (IsBody(_type))
    {
      return;
    }
    ...
    const PixelPattern pattern = PatternOf(SCANNER_COLOUR_TABLE[Byte(_type)]); // Into COL

    /*
     * The three high bytes ORed and the top two bits tested.
     *
    ...
    /*
     * The x coordinate, and the negation is the ordinary sign-magnitude one -- except that
     ...
     */
    bool carry = false; // The carry cleared
    std::uint8_t across = _ship.x.hi;
    if ((_ship.x.sgn & 0x80u) != 0u) // SC2, for a positive x
    {
    ...
    const std::uint8_t x1 = AddWithCarry(across, 123u, carry).value; // 123 added, into X1

    /*
     * The scanner's ellipse is drawn around a horizontal line, and this is where on that
    ...
    bool depthCarry = false;         // The carry cleared
    if ((_ship.z.sgn & 0x80u) != 0u) // SC3, for a positive z
    {
      depth = static_cast<std::uint8_t>(depth ^ 0xFFu);
      depthCarry = true; // SEC -- so the one's complement and the carry make a negation
    }

    const std::uint8_t ground = static_cast<std::uint8_t>(AddWithCarry(depth, 83u, depthCarry).value ^ 0xFFu);

    /*
     * And the height above that line, which is y_hi / 2 -- with the test the other way up.
     *
    ...
    bool heightCarry = false;        // The carry cleared
    if ((_ship.y.sgn & 0x80u) == 0u) // SCD6, for a negative y
    {
      height = static_cast<std::uint8_t>(height ^ 0xFFu);
      heightCarry = true;
    }

    std::uint8_t row = AddWithCarry(height, ground, heightCarry).value; // The ground row added

    // Clamped into the dashboard between 146 and 198, and the second comparison sees the
    ...
    const std::uint8_t y1 = row; // Into Y1

    // The ground row taken off, and the flags pushed -- how tall the stick is, and which way
    ...
    const CellCursor cursor = PlotBlock(_canvas, x1, y1, pattern);

    /*
     * The mask two entries on from the dot's, masked by the colour.
     *
    ...
    const std::uint8_t stickMask =
      static_cast<std::uint8_t>(MULTICOLOUR_MASK_TABLE[cursor.pixel + 2u] & PatternByte(pattern)); // Into X1

    // A zero height returns at once -- a ship exactly on the plane of flight has a dot and
    // no stick.
    if (stick.value == 0u)
    {
      return;
    ...
    {
      /*
       * Upwards, one `DEY` a row, and a step back of 320 bytes whenever that walks
      ...
      do
      {
        ...
        {
          within = 7u;
          address = static_cast<std::uint16_t>(address - 0x140u); // &140 off the address, in two bytes
        }
        else
        {
          --within;
        ...
        --remaining; // VLL1, while the count is not zero
      } while (remaining != 0u);
    ...
    /*
     * Downwards, and it steps TWICE before the first pixel.
     *
    ...
    {
      ++within;
      if (within == 8u) // The row index against 8
      {
        within = 0u;
        address = static_cast<std::uint16_t>(address + 0x140u); // &13F plus the set carry, in two bytes
      }
    };
    ...
    do
    {
      stepDown();
      _canvas.ExclusiveOr(static_cast<std::uint16_t>(address + within), stickMask);
      ++remaining; // VLL2, while the count is not zero
    } while (remaining != 0u);
  }
  ...
  void DrawCompassDot(Canvas& _canvas, const Compass& _compass, Picture* _picture) noexcept
  {
    ...
    // The compass's position and colour into the plotter's own bytes.
    // CPIX2 -- and the fall-through when the colour matches is `CPIX4`, because
  ...
  {
    // With the index at 0, 3 or 6, the axis's high byte.
    const SignMag24& axis = _planet.PositionAt(_at);
    _axes[_at] = axis.hi;

    // The top byte split into a magnitude and a sign, into the next two bytes.
  ...
  {
    // The three low bytes ORed together, with a bit forced on so the loop below is
    ...
    // And the three high bytes together, in the accumulator.
    std::uint8_t largest = static_cast<std::uint8_t>(_axes[1] | _axes[4] | _axes[7]);

    for (;;)
    {
      // One sixteen-bit doubling of the pair, and the bit that falls out of the top
      ...
      // Three sixteen-bit doublings, one per axis, and the last one's carry out is the
      // loop's other exit.
      ...
      if (step.carry) // Back to TAL2 while it does not carry
      {
        break;
      }
    }

    // Each high byte halved and its sign folded back on, three times. Seven bits of
    ...
    /*
     * And there is no `RTS`. `TAS2` runs straight on into `NORM`, so the three bytes above
  ...
  UnitVector LoadStationAxes(const Bubble& _bubble, K3Block& _axes) noexcept
  {
    ...
    return NormaliseAxes(_axes).vector; // TAS2, as a tail call
  }

  CompassOffset ScaleToCompass(std::uint8_t _value) noexcept
  {
    // The magnitude doubled into X, and the sign bit caught in the carry and rotated back
    ...
    constexpr std::uint8_t COMPASS_RADIUS = 20u; // 20 into Q -- the compass's radius in pixels
    const ScaledDivision divided = DivideAndScale(doubled.value, COMPASS_RADIUS);

    const std::uint8_t whole = divided.whole; // Out of P

    // Negate the offset for a negative coordinate, and hand back 255 as
    ...
    return {whole, 0u, divided.carry}; // A zero sign
  }

  void DrawCompass(Canvas& _canvas, Compass& _compass, UnitVector _towards, Picture* _picture) noexcept
  {
    /*
     * `SPS2` on the x axis, then 195 added to what it left in X.
     *
    ...
    // The original parks `ScaleToCompass`'s X in `T` before it can subtract from a literal;
    // the port subtracts the value where it stands.
    _compass.y = SubtractWithCarry(156u, down.offset, down.carry).value;

    // Yellow ahead and green behind, chosen on the sign of the z axis.
    _compass.pattern = ((_towards.z & 0x80u) != 0u) ? COMPASS_BEHIND : COMPASS_AHEAD;

    DrawCompassDot(_canvas, _compass, _picture); // DOT, as a tail call
  }

  void AimCompassAtStation(Canvas& _canvas, Compass& _compass, const Bubble& _bubble, K3Block& _axes, Picture* _picture) noexcept
  {
    const UnitVector towards = LoadStationAxes(_bubble, _axes);
    DrawCompass(_canvas, _compass, towards, _picture);          // The fall-through into SP2
  }

  void UpdateCompass(Canvas& _canvas, Compass& _compass, const Bubble& _bubble, Picture* _picture) noexcept
  {
    DrawCompassDot(_canvas, _compass, _picture); // Draw the old dot again to erase it

    K3Block axes{};

    // SP1 -- and `SSPR` is the station's entry in `MANY` (§6.58).
    ...
    const UnitVector towards = LoadPlanetAxes(_bubble, axes);
    DrawCompass(_canvas, _compass, towards, _picture);        // SP2, as a tail call
  }
