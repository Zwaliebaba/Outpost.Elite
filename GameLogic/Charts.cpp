...
void DrawCrosshairs(Canvas& _canvas, DrawWorkspace& _work, const Crosshairs& _at, std::uint8_t _view) noexcept
{
  ...
  _work.y2 = (bottom.value >= LONG_RANGE_BOTTOM && !ShortRange(_view)) ? std::uint8_t{ LONG_RANGE_BOTTOM - 1 }
                                                                      : bottom.value;
                         ...
                         const ChartView& _view, const SystemSeeds& _galaxy, ChartShapes* _shapes) noexcept
{
  ...
  for (int system = 0; system < 256; ++system)
  {
    ...
    if (AbsoluteDifference(x, _view.homeX) < SHORT_RANGE_SPAN_X
        && AbsoluteDifference(y, _view.homeY) < SHORT_RANGE_SPAN_Y)
    {
      /*
      ...
      const auto free = [&rowUsed](int _row) noexcept {
        return _row >= 0 && static_cast<std::size_t>(_row) < rowUsed.size()
               && rowUsed[static_cast<std::size_t>(_row)] == 0;
      };
      ...
      if (drawDisc && _shapes != nullptr)
      {
        ...
        const std::uint8_t radius =
          AddWithCarry(static_cast<std::uint8_t>(seeds.bytes[5] & 0x01u), 2, carry).value;
