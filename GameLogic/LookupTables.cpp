...
namespace Elite
{
  ...
  // "96 characters of eight rows, starting at space".
  static_assert(EXTENT<decltype(FONT_DATA)> == 96u * 8u, "the font is 96 characters of eight rows");

  // "the seven sprite definitions, 64 bytes each", and both numbers are `VideoState`'s.
  ...
  // Indexed by a screen row, and a row is a byte.
  static_assert(EXTENT<decltype(ROW_ADDRESS_LOW)> == 256u, "every y a byte can hold has a row address");

  // Indexed by a CHARACTER row, which is what `Canvas` counts.
  ...
  /*
   * "two identical entries per pixel so that a pair can be read at any offset",
  ...
  // TWFR and TWFL -- a horizontal line's two ends, one entry per x within a byte.
  ...
  // The four tables `COMIRQ1` indexes by `RASTCT` -- one entry per half of the split.
  ...
  /*
   * "sized by what indexes it, which is a ship TYPE", and a type runs 0 to
  ...
  // "two bytes an item, low byte first", for FOURTEEN items on this build.
  ...
  /*
   * "eight bytes of name and then the 77-byte data block, and thirteen more because
  ...
  /*
   * The eight per-effect tables `NOISE` indexes at `LDX` the effect number. They are eight
  ...
  // TRIBTA and TRIBMA -- "how many Trumble sprites to show" and "which sprite", by population.
  ...
  // TRIBDIR and TRIBDIRH -- "four entries because two bits are what index them".
